...
class CvForeignAdvisor:
	...
	def __init__(self):
		...
		self.Y_LEADER_CIRCLE_TOP = self.H_SCREEN / 2 - 45
		self.RADIUS_LEADER_ARC = self.Y_LEADER_CIRCLE_TOP - 25
		#self.RADIUS_LEADER_ARC = 480
		self.RADIUS_OVAL = 180
		self.CLOSE_BORDERS_ID = "CLOSE_BORDERS"
		self.close_borders = False
		self.CLOSE_BORDERS_OFF_TEXT = u"<font=2>" + "Border tension off" + u"</font>"
		self.CLOSE_BORDERS_ON_TEXT = u"<font=2>" + "Border tension on" + u"</font>"
		self.CLOSE_BORDERS_KEY = "close borders"
		self.HEATHEN_RELIGION_ID = "HEATHEN_RELIGION"
	...
	def drawRelations(self, bInitial):
		...
		#RevolutionDCM start
		#Center player leader head
		fLeaderArcTop = fLeaderTop + self.H_LEADER - 25
		#RevolutionDCM end
		...
		if (self.cbToggle % 2 == 0) :
			screen.setText(self.CLOSE_BORDERS_ID , "CB", self.CLOSE_BORDERS_OFF_TEXT, CvUtil.FONT_LEFT_JUSTIFY, self.CONTROL_AREA_X, self.CONTROL_AREA_Y + 0, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 0, 0)
		else :
			screen.setText(self.CLOSE_BORDERS_ID , "CB", self.CLOSE_BORDERS_ON_TEXT, CvUtil.FONT_LEFT_JUSTIFY, self.CONTROL_AREA_X, self.CONTROL_AREA_Y + 0, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 0, 0)
		if (self.hrToggle % 2 == 0) :
			screen.setText(self.HEATHEN_RELIGION_ID , "HR", self.HEATHEN_RELIGION_OFF_TEXT, CvUtil.FONT_LEFT_JUSTIFY, self.CONTROL_AREA_X, self.CONTROL_AREA_Y + self.CONTROL_AREA_SPACING, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 0, 0)
		else :
		...
		for iSelectedLeader in range(gc.getMAX_PLAYERS()):
			...
			if iSelectedLeader in self.listSelectedLeaders or (bNoLeadersSelected and bDisplayed):
				...
				if (iSelectedLeader in leaderMap):
					...
					#RevolutionDCM start
					#make oval
					fRadiusOval = fRadius + self.RADIUS_OVAL * abs(math.cos(thetaSelected))
					fXSelected = self.X_LEADER_CIRCLE_TOP - fRadiusOval * math.cos(thetaSelected)
					#RevolutionDCM end
				...
				#RevolutionDCM start
				#reference before assignment fix
				drawLine = False 
				
				for iPlayer in leaderMap.keys():
					if( iPlayer != iSelectedLeader):
						player = gc.getPlayer(iPlayer)
						...
						#find close border tension between selected player and this player
						grievanceText = aiAttitudes.getAttitudeString(iPlayer, iSelectedLeader)
						textIndex = grievanceText.rfind(self.CLOSE_BORDERS_KEY)
						grievanceText = aiAttitudes.getAttitudeString(iSelectedLeader, iPlayer)
						textIndex += grievanceText.rfind(self.CLOSE_BORDERS_KEY)
						if (self.close_borders and textIndex >= 0) :
							drawLine = True

						#find close border tension between selected player's vassal and this player
						for iVassalPlayer in range(gc.getMAX_PLAYERS()):
							vassalStatus = gc.getTeam(gc.getPlayer(iVassalPlayer).getTeam()).isVassal(gc.getPlayer(iSelectedLeader).getTeam())
							if (vassalStatus and (iVassalPlayer != iPlayer)) :
								grievanceText = aiAttitudes.getAttitudeString(iVassalPlayer, iPlayer)
								textIndex = grievanceText.rfind(self.CLOSE_BORDERS_KEY)
								grievanceText = aiAttitudes.getAttitudeString(iPlayer, iVassalPlayer)
								textIndex += grievanceText.rfind(self.CLOSE_BORDERS_KEY)
								if (self.close_borders and textIndex >= 0) : 
									drawLine = True

						#find religious tension between selected player and this player
						grievanceText = aiAttitudes.getAttitudeString(iPlayer, iSelectedLeader)
						textIndex = grievanceText.rfind(self.HEATHEN_RELIGION_KEY)
						if (self.heathen_religion and textIndex >= 0) :
							#Extra tests for religion
							playerSelectedSR = playerSelected.getStateReligion()
							playerSR = player.getStateReligion()
							if (playerSelectedSR != playerSR and playerSelectedSR >= 0 and playerSR >= 0) :
								drawLine = True

						if (not self.close_borders and not self.heathen_religion) :
							drawLine = True
						#RevolutionDCM end	
