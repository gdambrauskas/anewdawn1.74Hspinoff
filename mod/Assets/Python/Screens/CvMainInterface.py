		...
		if (not CyEngine().isGlobeviewUp() and pHeadSelectedCity):
			...
			if ((pHeadSelectedCity.getTeam() == gc.getGame().getActiveTeam()) or gc.getGame().isDebugMode()):
				...
				for i in range ( g_NumUnitClassInfos ):
					...
					if (pHeadSelectedCity.canTrain(eLoopUnit, False, not RoMOpt.isHideUntrainableUnits())):
						...
						if ( not pHeadSelectedCity.canTrain(eLoopUnit, False, False) or pHeadSelectedCity.getOwner() != gc.getGame().getActivePlayer()):
				...
				for item in ( g_BuildingList ):
					...
					if not isLimitedWonderClass(eBuildingClass):
						eLoopBuilding = gc.getCivilizationInfo(pHeadSelectedCity.getCivilizationType()).getCivilizationBuildings(eBuildingClass)

						if (pHeadSelectedCity.canConstruct(eLoopBuilding, False, not RoMOpt.isHideUnconstructableBuildings(), False)):
							screen.appendMultiListButton( "BottomButtonContainer", gc.getBuildingInfo(eLoopBuilding).getButton(), iRow, WidgetTypes.WIDGET_CONSTRUCT, eBuildingClass, -1, False )
							screen.show( "BottomButtonContainer" )
...
# BUG - Progress Bar - Tick Marks - end
				...
				#Afforess New Resource Slider
				screen.addTableControlGFC( "BonusListTable", 2, xResolution - 250, 115, 243, yResolution - 600, False, False, 32, 32, TableStyles.TABLE_STYLE_STANDARD )
...
#Afforess Hide Obsolete Buildings
					if ((pHeadSelectedCity.getNumBuilding(i) > 0) and not (gc.getActivePlayer().isModderOption(ModderOptionTypes.MODDEROPTION_HIDE_OBSOLETE_BUILDINGS) and pHeadSelectedCity.getNumActiveBuilding(i) == 0)):
