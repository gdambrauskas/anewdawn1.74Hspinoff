...
/************************************************************************************************/
	...
	if (wrapper.isUsingTaggedFormat())
	{
		...
		for(int i = 0; i < MAX_PLAYERS; i++)
		{
			...
			if ( m_aeCiv[i] == NO_CIVILIZATION && (m_aeSlotStatus[i] == SS_TAKEN || m_aeSlotStatus[i] == SS_COMPUTER || m_aeColor[i] != NO_COLOR) )
			{
...
/************************************************************************************************/
	...
	WRAPPER_WRITE(wrapper, "CvInitCore", m_eGameSpeed);
		...
		if (bDLLChanged || bAssetsChanged)
		{
			...
			for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
			{
				...
				if (kPlayer.isAlive() && kPlayer.isHuman())
				{
					...
					if (NULL != pInfo)
					{
						gDLL->getInterfaceIFace()->addPopup(pInfo, (PlayerTypes)iI, true, true);
					}
					m_svnRev = SVN_REV;
