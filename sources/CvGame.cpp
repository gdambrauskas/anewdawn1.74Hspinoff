...
//	This allows a 'space' for 'real' barbarians to be built befoer we use up the entire
//	simulatenous unit space of 8191 for any one player
#define	MAX_BARB_UNITS_FOR_SPAWNING	7000

// Public Functions...

#pragma warning( disable : 4355 )
CvGame::CvGame() : m_Properties(this)
{
...
/************************************************************************************************/
	...
	OutputDebugString(CvString::format("Stop profiling(false) after CvGanme::update()\n").c_str());
}
...
/*************************************************************************************************/
	...
	if (eIndex == GAMEOPTION_DARK_AGES)
	{
		return false;
	}
/*************************************************************************************************/
...
{
	PROFILE_FUNC();
	bool bUseThreads = GC.getDefineINT("USE_MULTIPLE_THREADS");
	...
	if (bUseThreads)
	{
		giProfilerDisabled++; // Deactivate non thread safe profiler
		...
		for(int j = 0; j < GC.getNumSpawnInfos(); j++)
		{
			...
			threads.push_back(new boost::thread(boost::bind(enumSpawnPlots, j, &(validPlots[j]))));
		}
	}
		...
		for (std::vector<CvPlot*>::iterator it = validPlots[j].begin(); it < validPlots[j].end(); it++)
		{
			...
			if ( getSorenRandNum( std::max(1,iLocalSpawnRate), "Unit spawn") == 0 )
			{
				...
				if ( localCount < std::max(1,localLandArea/std::max(1,spawnInfo.getMaxLocalDensity())) )
				{
					...
					CvUnit* pUnit = GET_PLAYER(BARBARIAN_PLAYER).initUnit(spawnInfo.getUnitType(), pPlot->getX_INLINE(), pPlot->getY_INLINE(), (UnitAITypes)kUnit.getDefaultUnitAIType());
					...
					for(int k = 0; k < spawnInfo.getNumSpawnGroup(); k++)
					{
						...
						pUnit = GET_PLAYER(BARBARIAN_PLAYER).initUnit(spawnInfo.getSpawnGroup(k), pPlot->getX_INLINE(), pPlot->getY_INLINE(), (UnitAITypes)kUnit.getDefaultUnitAIType());
						FAssertMsg(pUnit != NULL, "pUnit is expected to be assigned a valid unit object");
						pUnit->finishMoves();
						spawnCount++;
					}
	...
	if (bUseThreads)
		giProfilerDisabled--; // reactivate profiler (unless calling function has deactivated it as well)
}
...
/*                                                                                              */
/************************************************************************************************/
								CvUnit* pUnit = GET_PLAYER(BARBARIAN_PLAYER).initUnit(eBestUnit, pPlot->getX_INLINE(), pPlot->getY_INLINE(), eBarbUnitAI);
...
void CvGame::updateMoves()
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		if (player.isAlive())
		{
			...
			{
				//	For the human we want auto-moves last so the player can chnage
...
/************************************************************************************************/	
	...
	WRAPPER_READ(wrapper,"CvGame",&m_bNukesValid);

	WRAPPER_READ(wrapper,"CvGame",(int*)&m_eHandicap);
	WRAPPER_READ(wrapper,"CvGame",(int*)&m_ePausePlayer);
	WRAPPER_READ_CLASS_ENUM(wrapper,"CvGame",REMAPPED_CLASS_TYPE_UNITS,(int*)&m_eBestLandUnit);
...
/************************************************************************************************/
	...
	WRAPPER_READ_CLASS_ARRAY(wrapper,"CvGame",REMAPPED_CLASS_TYPE_TECHS,GC.getNumTechInfos(), m_abTechCanFoundReligion);
/************************************************************************************************/
/* LIMITED_RELIGIONS               END                                                          */
/************************************************************************************************/
	...
	if ( wrapper.isUsingTaggedFormat() )
	{
		WRAPPER_READ_CLASS_ENUM_ARRAY(wrapper,"CvGame",REMAPPED_CLASS_TYPE_BUILDINGS,m_iShrineBuildingCount, m_aiShrineBuilding);
		WRAPPER_READ_CLASS_ENUM_ARRAY(wrapper,"CvGame",REMAPPED_CLASS_TYPE_RELIGIONS,m_iShrineBuildingCount, m_aiShrineReligion);
	}
	else
	{
...
/************************************************************************************************/
	...
	{
		WRAPPER_WRITE_DECORATED(wrapper, "CvGame", *e_it, "InactiveTrigger");
	}
...
void CvGame::loadPirateShip(CvUnit* pUnit)
{
	...
	for (int iI = 0; iI < pUnit->cargoSpace(); iI++)
	{
		...
		if (eBestUnit != NO_UNIT)
		{
			CvUnit* pPirate = GET_PLAYER(BARBARIAN_PLAYER).initUnit(eBestUnit, pUnit->plot()->getX_INLINE(), pUnit->plot()->getY_INLINE(), UNITAI_ATTACK);
...
void CvGame::recalculateModifiers()
{
	...
	//	Inhibiot plot group manipulation until we rebuild at the end of everything else
	for(int iI = 0; iI < MAX_PLAYERS; iI++ )
	{
		if ( GET_PLAYER((PlayerTypes)iI).isAlive() )
		{
