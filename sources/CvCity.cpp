...
//	Dynamic cache used to store the availability of upgrades
static UnitTypes* eCachedAllUpgradesResults = NULL;
static bool* bCachedAllUpgradesResultsAvailable = NULL;
#ifdef CAN_TRAIN_CACHING
static const CvCity* pCachedAllUpgradesCity = NULL;
#endif
		...
		for(int iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if ( GET_PLAYER((PlayerTypes)iI).isAlive() )
			{
				if ( originalTradeNetworkConnectivity[iI] == NULL )
				{
					...
					{
						GET_PLAYER((PlayerTypes)iI).updatePlotGroups(area());
					}
				}
				else
				{
...
void CvCity::doTurn()
{
	...
	CvCityAI::FlushBuildingValueCache();
...
UnitTypes CvCity::allUpgradesAvailable(UnitTypes eUnit, int iUpgradeCount) const
{
	...
	if ( iUpgradeCount == 0 )
	{
		//	Create cache arrays of unit results if not yet done
		if ( eCachedAllUpgradesResults == NULL )
		{
			eCachedAllUpgradesResults = new UnitTypes[GC.getNumUnitInfos()];
			bCachedAllUpgradesResultsAvailable = new bool[GC.getNumUnitInfos()];
		}

#ifdef CAN_TRAIN_CACHING
		//	If can train caching is in use (which means values get invalidated correctly by calls
		//	on that cache) we can retian results for the same city previously calculated
		if ( pCachedAllUpgradesCity != this )
#endif
		{
			memset(bCachedAllUpgradesResultsAvailable,0,GC.getNumUnitInfos());

#ifdef CAN_TRAIN_CACHING
			pCachedAllUpgradesCity = this;
#endif
		}
	}
	else if ( bCachedAllUpgradesResultsAvailable[eUnit] )
	{
		return eCachedAllUpgradesResults[eUnit];
	}
...
/************************************************************************************************/
	...
	/************************************************************************************************/
		...
		if (iUpgradeCount > 0)
		{
			...
			bCachedAllUpgradesResultsAvailable[eUnit] = true;
			eCachedAllUpgradesResults[eUnit] = eResult;
		}
		else
		{
...
void CvCity::populateCanTrainCache(void) const
{
	PROFILE_FUNC();
...
void CvCity::invalidateCachedCanTrainForUnit(UnitCombatTypes eUnitCombat) const
{
	if ( m_canTrainCachePopulated )
	{
		...
		{
			std::map<UnitCombatTypes,bool>::iterator itr = m_canTrainCache.find(eUnitCombat);
			if ( itr != m_canTrainCache.end() )
			{
				itr->second = false;	//	In map but with false => dirty
			}

			for(std::map<UnitTypes,bool>::iterator unitItr = m_canTrainCacheUnits.begin(); unitItr != m_canTrainCacheUnits.end(); unitItr++)
			{
				if ( unitItr->second )
				{
					CvUnitInfo& kUnit = GC.getUnitInfo(unitItr->first);

					if ( kUnit.getUnitCombatType() == (int)eUnitCombat )
					{
	...
	if ( bCachedAllUpgradesResultsAvailable != NULL )
	{
		for(int iI = 0; iI < GC.getNumUnitInfos(); iI++)
		{
			if ( bCachedAllUpgradesResultsAvailable[iI] )
			{
				CvUnitInfo& kUnit = GC.getUnitInfo((UnitTypes)iI);

				if ( kUnit.getUnitCombatType() == (int)eUnitCombat )
				{
					bCachedAllUpgradesResultsAvailable[iI] = false;
				}
			}
		}
	}
...
bool CvCity::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreUpgrades) const
{
	...
	if ( !bContinue && !bTestVisible && !bIgnoreCost && !bIgnoreUpgrades )
	{
		if ( m_canTrainCachePopulated )
		{
			...
			std::map<UnitTypes,bool>::iterator itr = m_canTrainCacheUnits.find(eUnit);
...
CvCity* CvCity::g_canConstructCacheCity = NULL;
std::map<int,bool>*	CvCity::g_bCanConstruct = NULL;

void CvCity::FlushCanConstructCache(BuildingTypes eBuilding)
{
	if ( eBuilding == NO_BUILDING )
	{
		g_canConstructCacheCity = NULL;
		if ( g_bCanConstruct != NULL )
		{
			delete g_bCanConstruct;
			g_bCanConstruct = NULL;
		}
	}
	else if ( g_bCanConstruct != NULL )
	{
		(*g_bCanConstruct).erase(eBuilding);
	}
}

void CvCity::NoteBuildingNoLongerConstructable(BuildingTypes eBuilding) const
{
	if ( g_canConstructCacheCity == this )
	{
		(*g_bCanConstruct)[eBuilding] = false;
	}
}
...
bool CvCity::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreAmount) const
{
	...
	else if ( !bContinue && !bTestVisible && !bIgnoreCost && !bIgnoreAmount )
	{
		bool bResult;
		...
		if ( g_canConstructCacheCity != this )
		{
			...
			OutputDebugString(CvString::format("Rebuilding canConstruct cache for City %S\n", getName().GetCString()).c_str());
			FlushCanConstructCache();

			g_bCanConstruct = new std::map<int,bool>();
			g_canConstructCacheCity = (CvCity*)this;
		}
		std::map<int,bool>::const_iterator itr = g_bCanConstruct->find(eBuilding);
		if ( itr == g_bCanConstruct->end() )
		{
			bResult = canConstructInternal(eBuilding, bContinue, bTestVisible, bIgnoreCost, bIgnoreAmount);
			{
				MEMORY_TRACK_EXEMPT()

				(*g_bCanConstruct)[eBuilding] = bResult;
			}

			//CvBuildingInfo& kBuilding = GC.getBuildingInfo(eBuilding);
			//OutputDebugString(CvString::format("canConstruct - cached building %d (%S), result %d\n", eBuilding, kBuilding.getDescription(), bResult).c_str());
		}
		else
		{
			bResult = itr->second;
			...
			CvBuildingInfo& kBuilding = GC.getBuildingInfo(eBuilding);
			//OutputDebugString(CvString::format("canConstruct result for City %S, building %S: %d\n", getName().GetCString(), kBuilding.getDescription(),(int)bResult).c_str());
	...
	{
		return canConstructInternal(eBuilding, bContinue, bTestVisible, bIgnoreCost, bIgnoreAmount);
	}
}
...
/*                                                                                              */
/************************************************************************************************/
bool CvCity::canConstructInternal(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreAmount) const
{
	PROFILE_FUNC()
...
/************************************************************************************************/
/* Koshling - moved this so non-met vicvinity required buildings get hidden in city screen even */
...
const wchar* CvCity::getProductionName() const
{
	...
	if (pOrderNode != NULL)
	{
		...
		{
		case ORDER_TRAIN:
			return GC.getUnitInfo((UnitTypes) pOrderNode->m_data.iData1).getDescription(getCivilizationType());
			break;
...
void CvCity::processBuilding(BuildingTypes eBuilding, int iChange, bool bObsolete)
{
	...
	if ((!(GET_TEAM(getTeam()).isObsoleteBuilding(eBuilding)) && (m_paiBuildingReplaced[eBuilding] == 0 || (m_recalcBuilding != MAX_INT && iChange < 0))) || bObsolete)
	{
		if (iChange > 0)
		{
...
#define	MAX_SURPLASS	500
	...
	if ( surplass == -1 )
	{
		surplass = foodDifference(true, false);
	}

	//	Nothign wasted if there is no surplass
	if ( surplass <= 0 )
	{
		return 0;
	}
	...
	else
	{
		...
		{
			calculatedWaste[surplass] = foodWastage(surplass-1) + ((float)1 - ((float)0.05 + pow((float)0.95, surplass))/(float)1.05);
...
int CvCity::foodDifference(bool bBottom, bool bIncludeWastage) const
{
	int iDifference;
...
void CvCity::changeBuildingReplacementCount(BuildingTypes eBuilding, bool bAdd)
{
	...
	for(int iJ = 0; iJ < GC.getNumBuildingInfos(); iJ++)
	{
		if (GC.getBuildingInfo((BuildingTypes)iJ).isReplaceBuildingClass(GC.getBuildingInfo(eBuilding).getBuildingClassType()))
		{
			...
			if ( bHad != bHas )
			{
				processBuilding((BuildingTypes)iJ, bHas ? 1 : -1);
			}
		}
	}
}
...
long CvCity::getRealPopulation() const
{
	...
	//	Koshling - using table providign by Praetyre to give more realistic results
...
int CvCity::getSavedMaintenanceTimes100ByBuilding(BuildingTypes eBuilding) const
{
	...
	if (iModifier != 0 && !isDisorder() && !isWeLoveTheKingDay())
	{
		...
		int baseMaintenance = calculateBaseMaintenanceTimes100();
		int iOldModifiedBaseMaintenance = std::max(0, baseMaintenance * (getMaintenanceModifier() + 100) / 100);
		int iNewModifiedBaseMaintenance = std::max(0, baseMaintenance * (getMaintenanceModifier() + iModifier + 100) / 100);
...
int CvCity::getAdditionalHappinessByCivic(CivicTypes eCivic, bool bDifferenceToCurrent, bool bCivicOptionVacuum, ReligionTypes eStateReligion, int iExtraPop, int iMilitaryHappinessUnits) const
{
	...
	int iI = 0;
			...
			if (kCivic.getCityLimit() > 0 && kCivic.getCityOverLimitUnhappy() > 0 && kOwner.getNumCities() > kCivic.getCityLimit())
			{
				iHappy -= kCivic.getCityOverLimitUnhappy()*(kOwner.getNumCities() - kCivic.getCityLimit());
			}
...
/************************************************************************************************/
	...
	iBaseCommerceRate += getBonusCommercePercentChanges(eIndex);
...
int CvCity::getAdditionalCommerceTimes100ByBuilding(CommerceTypes eIndex, BuildingTypes eBuilding)
{
	...
	int iExtraTimes100 = ((iModifier + iExtraModifier) * (iExtraRateTimes100 + iRateTimes100) / 100) - (iModifier * iRateTimes100 / 100);
...
void CvCity::setNumRealBuildingTimed(BuildingTypes eIndex, int iNewValue, bool bFirst, PlayerTypes eOriginalOwner, int iOriginalTime)
{
	...
	if (iChangeNumRealBuilding != 0)
	{
		...
		if ( m_paiNumRealBuilding[eIndex] != iNewValue )
		{
			...
			CvCityAI::FlushBuildingValueCache();
			FlushCanConstructCache();	//	Flush all
#ifdef YIELD_VALUE_CACHING
		...
		if (iChangeNumRealBuilding > 0)
		{
			if (bFirst)
			{
				...
				if (GC.getBuildingInfo(eIndex).isAllowsNukes())
				{
					...
					{
						GET_TEAM(getTeam()).changeNukesValidCount(iChangeNumRealBuilding);
					}
					else 
					{
...
/************************************************************************************************/
			if( gCityLogLevel >= 1 )
			{
				...
				logBBAI("    City %S pushes production of unit %S with UNITAI %S", getName().GetCString(), GC.getUnitInfo((UnitTypes) iData1).getDescription(getCivilizationType()), szString.GetCString() );
			}
/************************************************************************************************/
/* BETTER_BTS_AI_MOD                       END                                                  */
/************************************************************************************************/
...
void CvCity::popOrder(int iNum, bool bFinish, bool bChoose)
{
	...
	case ORDER_TRAIN:
		...
		if (bFinish)
		{
			...
			OutputDebugString(CvString::format("Unit %d (%S) built\n", eTrainUnit, GC.getUnitInfo(eTrainUnit).getDescription()).c_str());
...
/************************************************************************************************/	
	...
	if ( wrapper.isUsingTaggedFormat() )
	{
		...
		while (pNode != NULL)
		{
			...
			{
			case ORDER_TRAIN:
				bDeleteNode = ((pNode->m_data.iData1 = wrapper.getNewClassEnumValue(REMAPPED_CLASS_TYPE_UNITS,pNode->m_data.iData1, true)) == -1);
				break;
			case ORDER_CONSTRUCT:
...
/************************************************************************************************/
	...
	WRAPPER_WRITE_ARRAY(wrapper, "CvCity", NUM_COMMERCE_TYPES, m_aiSpecialistCommerce);
...
void CvCity::getVisibleBuildings(std::list<BuildingTypes>& kChosenVisible, int& iChosenNumGenerics)
{
	...
	for(int i = 0; i < iNumBuildings; i++)
	{
		...
		{
			switch(GC.getDefineINT("SHOW_BUILDINGS_LEVEL"))
			{
			case 0:
				//	No building art displayed
				continue;
			case 1:
				//	Wonders only
				if ( !isLimitedWonderClass((BuildingClassTypes)GC.getBuildingInfo(eCurType).getBuildingClassType()) )
				{
					continue;
				}
				break;
			case 2:
				//	Wonders and defences only
				if ( !isLimitedWonderClass((BuildingClassTypes)GC.getBuildingInfo(eCurType).getBuildingClassType()) &&
					 GC.getBuildingInfo(eCurType).getDefenseModifier() <= 0 )
				{
					continue;
				}
				break;
...
void CvCity::checkBuildings(bool bVicinityBonus, bool bCivics, bool bWar, bool bPower, bool bPopulation)
{
				...
				if (getNumBuilding(eLoopBuilding) > 0 || isDisabledBuilding(eLoopBuilding))
				{
					...
					if (bRestoreBuildings && isDisabledBuilding(eLoopBuilding))
					{
						...
						if (!GET_PLAYER(getOwnerINLINE()).isModderOption(MODDEROPTION_IGNORE_DISABLED_ALERTS))
						{
							...
							gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, NULL, MESSAGE_TYPE_MINOR_EVENT, kBuilding.getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), getX_INLINE(), getY_INLINE(), true, true);
						}
					}
					else if (bObsoleteBuildings && !isDisabledBuilding(eLoopBuilding))
					{
						...
						if (!GET_PLAYER(getOwnerINLINE()).isModderOption(MODDEROPTION_IGNORE_DISABLED_ALERTS))
						{
							gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, NULL, MESSAGE_TYPE_MINOR_EVENT, kBuilding.getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"), getX_INLINE(), getY_INLINE(), true, true);
						}
					}
				}
			}
...
void CvCity::calculateExtraTradeRouteProfit(int iExtra, int* &aiTradeYields) const
{
	...
	if (!isDisorder() && !isPlundered())
	{
		iTradeRoutes = std::min(getTradeRoutes() + std::max(0, iExtra), GC.getDefineINT("MAX_TRADE_ROUTES"));
	...
	for (iI = 0; iI < std::min(getTradeRoutes(), GC.getDefineINT("MAX_TRADE_ROUTES")); iI++)
	{
