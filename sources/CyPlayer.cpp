...
/*                                                                                              */
/************************************************************************************************/
int CyPlayer::getDarkAgeTurns()
{
	return m_pPlayer ? m_pPlayer->getDarkAgeTurns() : -1;
}

int CyPlayer::getDarkAgeLength()
{
	return m_pPlayer ? m_pPlayer->getDarkAgeLength() : -1;
}

bool CyPlayer::isDarkAge()
{
	return m_pPlayer ? m_pPlayer->isDarkAge() : false;
}

void CyPlayer::changeDarkAgeTurns(int iChange)
{
	if (m_pPlayer)
		m_pPlayer->changeDarkAgeTurns(iChange);
}

bool CyPlayer::isDarkAgeCapable()
{
	return m_pPlayer ? m_pPlayer->isDarkAgeCapable() : false;
}

int CyPlayer::getDarkAgePointsFinal()
{
	if (m_pPlayer)
	{
		CvDarkAgePoints points;
		m_pPlayer->calculateDarkAgePoints(&points);
		return (int)points.m_iTotalFinal;
	}
	return -1;
}

bool CyPlayer::canHaveSlaves()
{
	return m_pPlayer ? m_pPlayer->canHaveSlaves() : false;
}
...
/* Afforess	                     END                                                            */
/************************************************************************************************/
int CyPlayer::getCurrentAgeSegment() const
{
	return m_pPlayer ? m_pPlayer->getCurrentAgeSegment() : 0;
}

void CyPlayer::setCurrentAgeSegment(int /*AgeSegments*/ eNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setCurrentAgeSegment((AgeSegments)eNewValue);
}

int CyPlayer::getCurrentCulturalAge() const
{
	return m_pPlayer ? m_pPlayer->getCurrentCulturalAge() : 0;
}

void CyPlayer::setCurrentCulturalAge(int /*CulturalAgeTypes*/ eNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setCurrentCulturalAge((CulturalAgeTypes)eNewValue);
}
