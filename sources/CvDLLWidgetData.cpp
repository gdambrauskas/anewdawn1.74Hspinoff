...
/************************************************************************************************/
						...
						GET_PLAYER(pHeadSelectedUnit->getOwnerINLINE()).getNumCities() >= GET_PLAYER(pHeadSelectedUnit->getOwnerINLINE()).getCityLimit())
					{
						...
						for (iI = 0; iI < GC.getNumCivicOptionInfos(); iI++)
						{
							...
							{
								if (GC.getCivicInfo((CivicTypes)GET_PLAYER(pHeadSelectedUnit->getOwnerINLINE()).getCivics((CivicOptionTypes)iI)).getCityLimit() > 0)
								{
				...
				if (pMissionCity != NULL)
				{
					...
					while (pSelectedUnitNode != NULL)
					{
						...
						if (pSelectedUnit->canHurryFood(pMissionPlot))
						{
							...
							{
								szBuffer.append(NEWLINE);
								szBuffer.append(gDLL->getText("TXT_KEY_ACTION_EXTRA_FOOD", pSelectedUnit->getHurryProduction(pMissionPlot), pcKey));
							}
						break;
						}
						pSelectedUnitNode = gDLL->getInterfaceIFace()->nextSelectionListNode(pSelectedUnitNode);
...
// BUG - Fortify/Sleep All Action - end
			...
			if ( eMission > MISSION_PRETARGET_NUKE) // Outcome missions
			{
				...
				while (pSelectedUnitNode != NULL)
				{
					...
					CvOutcomeList* pOutcomeList = pSelectedUnit->getUnitInfo().getActionOutcomeList(eMission);
					if (pOutcomeList)
					{
						if (pOutcomeList->isPossible(*pSelectedUnit))
						{
							pOutcomeList->buildDisplayString(szBuffer, *pSelectedUnit);
							break;
						}
					}
...
void CvDLLWidgetData::parseBuildingFilterHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	if (pHeadSelectedCity != NULL)
	{
		if (pHeadSelectedCity->getBuildingListFilterActive(eFilter))
		{
			szBuffer.append(gDLL->getText("TXT_KEY_MISC_TURN_OFF"));
		}
		else
...
void CvDLLWidgetData::parseUnitFilterHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	if (pHeadSelectedCity != NULL)
	{
		if (pHeadSelectedCity->getUnitListFilterActive(eFilter))
		{
			szBuffer.append(gDLL->getText("TXT_KEY_MISC_TURN_OFF"));
		}
		else
