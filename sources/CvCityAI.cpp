...
BuildingValueCache* CvCityAI::cachedBuildingValues = NULL;
...
{
	if ( m_requestedEscorts > 0 && eUnitAIType == UNITAI_CITY_COUNTER )
	{
		m_requestedEscorts--;
	}
}
...
/*                                                                                              */
/*This should scale with gamespeed                                                              */
/************************************************************************************************/
...
/************************************************************************************************/
	...
	UnitTypes eBestWorker = AI_bestUnitAI(UNITAI_WORKER, false, NO_ADVISOR, true);
	if ( eBestWorker != NO_UNIT )
	{
		...
		//	Empirical ratio needed to distibguish C2C gatherer, but should hopefully
...
/********************************************************************************/
	...
	int iPlotCityDefenderCount = plot()->plotCount(PUF_isUnitAIType, UNITAI_CITY_DEFENSE, -1, getOwnerINLINE(), NO_TEAM, NULL, -1, -1, 2);
/********************************************************************************/
/* 	City Defenders						24.07.2010				Fuyu			*/
/********************************************************************************/
	int iPlotOtherCityAICount = plot()->plotCount(PUF_canDefend, -1, -1, getOwnerINLINE(), NO_TEAM, PUF_isCityAIType, -1, -1, 2) - iPlotCityDefenderCount;
...
/********************************************************************************/
	...
	if ( iHappyness < -1 || iHappyness <= -getPopulation()/2 )
	{
		...
		if ( GET_PLAYER(getOwner()).getHappyPerMilitaryUnit() > 0 &&
			 iPlotCityDefenderCount + iPlotOtherCityAICount < (AI_minDefenders() + iPlotSettlerCount + 3) )
		{
			if (AI_chooseUnit(UNITAI_CITY_DEFENSE))
			{
				if( gCityLogLevel >= 2 ) logBBAI("      City %S builds defender for emergency military happyness", getName().GetCString());
				return;
			}
		}
	}
	...
	//minimal defense.
	if (iPlotCityDefenderCount <= iPlotSettlerCount || m_requestedEscorts > 0)
	{
		...
		if (AI_chooseUnit(bestAIType))
		{
			// BBAI TODO: Does this work right after settler is built???
			if( gCityLogLevel >= 2 ) logBBAI("      City %S uses escort best AI", getName().GetCString());
			return;
		}
...
//Fuyu bIgnoreNotUnitAIs
		...
		{
			if (AI_chooseUnit(bestAIType, -1, true))
			{
				// Yes it probably works but it should never happen in the first place
				if( gCityLogLevel >= 2 ) logBBAI("      City %S uses escort best AI (IgnoreNotUnitAIs)", getName().GetCString());
				return;
			}
		}
...
/********************************************************************************/
	...
	if ( iHappyness < 0 )
	{
		...
		//	normally consider
		if ( GET_PLAYER(getOwner()).getHappyPerMilitaryUnit() > 0 &&
			 iPlotCityDefenderCount + iPlotOtherCityAICount < (AI_minDefenders() + iPlotSettlerCount + 1) )
		{
			if (AI_chooseUnit(UNITAI_CITY_DEFENSE))
			{
				if( gCityLogLevel >= 2 ) logBBAI("      City %S builds defender for non-emergency military happyness", getName().GetCString());
				return;
			}
		}
	}
...
//TB Build Priority Mod Begin    
	{
		...
		if (AI_chooseBuilding(BUILDINGFOCUS_PRODUCTION, 10))
		{
			return;
		}

		if (AI_chooseBuilding(BUILDINGFOCUS_FOOD, 10))
		{
			return;
		}
	}
...
/********************************************************************************/
	...
	//Afforess first attempt to build resource factories in order to unlock units - only try for high value ones here
	if (AI_constructResourceFactories(25, 512)) {
		return;
	}
...
/********************************************************************************/
		...
		if( !(bLandWar && iWarSuccessRatio < -30) && !bDanger && !bFinancialTrouble )
		{
			if (kPlayer.AI_getNumTrainAIUnits(UNITAI_ATTACK_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_PIRATE_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_RESERVE_SEA) < std::min(3,kPlayer.getNumCities()))
			{
				if ((bMaybeWaterArea && bWaterDanger)
					|| (pWaterArea != NULL &&
						bPrimaryArea &&
						kPlayer.AI_countNumAreaHostileUnits(pWaterArea, true, false, false, false) > 0) )
				{
					...
					{
						OutputDebugString("Local navy suffices\n");
					}
				}
			}
	...
	//Afforess second attempt to build resource factories in order to unlock units
	if (AI_constructResourceFactories(50, 400)) {
		return;
	}
...
//TB Build Mod end	
	...
	//Afforess third attempt to build resource factories in order to unlock units
	if (AI_constructResourceFactories(75, 256)) {
		return;
	}
...
/********************************************************************************/
	if( !(bLandWar && iWarSuccessRatio < 0) && !bDanger && !bBestWorkerIsOneShot)	//	KOSHLING - don't build frivolous one-shot workers like this
	{
...
/********************************************************************************/
    ...
    if (!bImportantCity || bLandWar || bAssault)
    {
        ...
        {
			int iExplorerDeficit = kPlayer.AI_neededExplorers(pArea) - kPlayer.AI_totalAreaUnitAIs(pArea, UNITAI_EXPLORE);
			int iHunterDeficit = kPlayer.AI_neededHunters(pArea) - kPlayer.AI_totalAreaUnitAIs(pArea, UNITAI_HUNTER);

			if ( iExplorerDeficit >= iHunterDeficit && iExplorerDeficit > 0 )
			{
				//	If we are just pumping out explorer units and havign them die fast
				//	go for EXP giving buildings first
				UnitTypes eBestUnit = AI_bestUnitAI(UNITAI_EXPLORE, false, NO_ADVISOR, false);
			...
			if (iHunterDeficit > 0)
			{
				...
				//	go for EXP giving buildings first
				UnitTypes eBestUnit = AI_bestUnitAI(UNITAI_HUNTER, false, NO_ADVISOR, false);
				if ( eBestUnit != NO_UNIT )
				{
					//	What is the cost of one of these units?
					int iCost = (GC.getUnitInfo(eBestUnit).getProductionCost() * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getTrainPercent()) / 100;
...
/************************************************************************************************/
	if (!bAlwaysPeace && (GC.getGameINLINE().isOption(GAMEOPTION_RUTHLESS_AI) || kPlayer.isEnabledMAD() || (GC.getGame().getSorenRandNum(10, "AI consider Nuke") == 0)))
	{
		if(!bFinancialTrouble)
		{
...
/************************************************************************************************/ 
	...
	//Afforess fourth (and final) attempt to build resource factories in order to unlock units
	if (AI_constructResourceFactories(100, 144)) {
		return;
	}
...
//TB Build Mod end	
	...
	// Koshling in early game moved optional non-wartime attack unit builds below economy
	// don't build frivolous things if this is an important city unless we at war
    if ((!bImportantCity || bLandWar || bAssault) && kPlayer.getCurrentEra() == 0)
    {
        if (bPrimaryArea)
        {
            if (kPlayer.AI_totalAreaUnitAIs(pArea, UNITAI_ATTACK) == 0)
	...
	if (iUnitCostPercentage < (iMaxUnitSpending + 4) && (!bImportantCity || bDefenseWar) )
	{
		if( bLandWar || bAssault || (iFreeAirExperience > 0) || (GC.getGame().getSorenRandNum(3, "AI train air") == 0) )
		{
			...
			if ((iBestAirValue + iBestMissileValue) > 0)
			{
				...
				if (iAircraftHave < iAircraftNeed)
				{
					if (AI_chooseLeastRepresentedUnit(airUnitTypes))
					{
						return;
				...
				if( 3*iFightersHave < iAircraftNeed * 2 )
				{
	...
	if (!bAlwaysPeace && (kPlayer.AI_isDoStrategy(AI_STRATEGY_OWABWNW) || kPlayer.isEnabledMAD() || (GC.getGame().getSorenRandNum(chance, "AI consider Nuke") == 0)))
	{
		if( !bFinancialTrouble )
		{
			int iTotalNukes = kPlayer.AI_totalUnitAIs(UNITAI_ICBM);
...
//TB Build Mod (Remove unnecessary elements)
	...
	if (plot()->plotCount(PUF_isUnitAIType, UNITAI_CITY_COUNTER, -1, getOwnerINLINE(), NO_TEAM, NULL, -1, -1, 2) == 0)
	{
		if (AI_chooseUnit(UNITAI_CITY_COUNTER))
		{
			return;
	...
	if (plot()->plotCount(PUF_isUnitAIType, UNITAI_PILLAGE_COUNTER, -1, getOwnerINLINE(), NO_TEAM, NULL, -1, -1, 2) == 0)
	{
		if (AI_chooseUnit(UNITAI_PILLAGE_COUNTER))
		{
			return;
		}
	}
					   ...
					   isLimitedWonderClass((BuildingClassTypes)iI)))
				{
					if ((eIgnoreAdvisor == NO_ADVISOR) || (GC.getBuildingInfo(eLoopBuilding).getAdvisorType() != eIgnoreAdvisor))
					{
						if (canConstruct(eLoopBuilding))
						{
							...
							{
								BuildingTypes eFreeBuilding = (BuildingTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(GC.getBuildingInfo(eLoopBuilding).getFreeAreaBuildingClass());
								if (NO_BUILDING != eFreeBuilding)
								{
									...
									if ( getNumBuilding(eFreeBuilding) == 0 )
									{
										weighting++;
									}
									iValue += (AI_buildingValue(eFreeBuilding, iFocusFlags) * weighting)/100;
								}
							}

							if (GC.getBuildingInfo(eLoopBuilding).getFreeTradeRegionBuildingClass() != NO_BUILDINGCLASS)
							{
								BuildingTypes eFreeBuilding = (BuildingTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(GC.getBuildingInfo(eLoopBuilding).getFreeTradeRegionBuildingClass());
								if (NO_BUILDING != eFreeBuilding)
								{
...
*/
											iTempValue = GC.getGameINLINE().getSorenRandNum(GET_PLAYER(getOwnerINLINE()).getWonderConstructRand(), "Wonder Construction Rand");
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
										}
...
int CvCityAI::AI_buildingValueThreshold(BuildingTypes eBuilding, int iFocusFlags, int iThreshold, bool bMaximizeFlaggedValue)
{
	PROFILE_FUNC();

	int result = GetBuildingValue(eBuilding, iFocusFlags, iThreshold, bMaximizeFlaggedValue);
...
int CvCityAI::AI_buildingValueThresholdOriginal(BuildingTypes eBuilding, int iFocusFlags, int iThreshold, bool bMaximizeFlaggedValue)
{
	PROFILE_FUNC();
...
#ifdef _DEBUG
	OutputDebugString(CvString::format("AI_buildingValueThreshold: %S (%08x, %d) @ %S\n", kBuilding.getDescription(), iFocusFlags, iThreshold, getName().GetCString()).c_str());
	if ( !canConstruct(eBuilding) )
	{
		OutputDebugString("    Requested building cannot be constructed\n");
	}
#endif
...
*/
				...
				else
				{
					...
					//	KOSHLING - Bug fix - the above is pretty nasty anyway, but critically
					//	it inverts negative values of 'iBuildingActualHappiness' which are then used
					//	with the wrong sign later in the loop/on the next loop iteration.
					//	For now re-invert it but should probably clean up the above more generally
					if (bIsNegative)
					{
						iBuildingActualHappiness = -iBuildingActualHappiness;
					}
...
/************************************************************************************************/
			...
			if ((iFocusFlags & BUILDINGFOCUS_EXPERIENCE) || (iPass > 0))
			{
				...
				iValue += iTempValue;
				bool bDevelopingCity = isDevelopingCity();
				if (!bDevelopingCity || isCapital())
				{
					BonusTypes eFreeBonus = (BonusTypes)kBuilding.getFreeBonus();

					for (iI = 0; iI < GC.getNumUnitInfos(); iI++)
					{
						CvUnitInfo& kUnit = GC.getUnitInfo((UnitTypes)iI);
						bool bUnitIsEnabled = (kUnit.getPrereqBuilding() == eBuilding);

						if ( !bUnitIsEnabled &&
							 eFreeBonus != NO_BONUS &&
							 !hasBonus(eFreeBonus) )
						{
							if ( kUnit.getPrereqAndBonus() == eFreeBonus )
							{
								bUnitIsEnabled = true;
							}
							else
							{
								...
								{
									if ( kUnit.getPrereqOrBonuses(iK) == eFreeBonus )
									{
										bUnitIsEnabled = true;
										break;
									}
						...
						if ( !bUnitIsEnabled )
						{
							if ( kUnit.getPrereqAndBonus() != NO_BONUS || kUnit.getPrereqOrBonuses(0) != NO_BONUS )
							{
								for( int iJ = 0; iJ < kBuilding.getNumExtraFreeBonuses(); iJ++ )
								{
									BonusTypes eExtraFreeBonus = kBuilding.getExtraFreeBonus(iJ);

									if ( kUnit.getPrereqAndBonus() == eExtraFreeBonus )
									{
										bUnitIsEnabled = true;
										break;
									}
									else
									{
										for (int iK = 0; iK < GC.getNUM_UNIT_PREREQ_OR_BONUSES(); iK++)
										{
											if ( kUnit.getPrereqOrBonuses(iK) == eExtraFreeBonus )
											{
												bUnitIsEnabled = true;
												break;
											}
...
/************************************************************************************************/
				...
				for(iI = 0; iI < kBuilding.getNumExtraFreeBonuses(); iI++)
				{
					iValue += (kOwner.AI_bonusVal((BonusTypes)(kBuilding.getExtraFreeBonus(iI)), 1) *
								 ((kOwner.getNumTradeableBonuses((BonusTypes)(kBuilding.getExtraFreeBonus(iI))) == 0) ? 2 : 1) *
								 (iNumCities + kBuilding.getExtraFreeBonusNum(iI)));
				}
...
/************************************************************************************************/
					...
					for (iI = 0; iI < MAX_TEAMS; iI++)
					{
						if (GET_TEAM((TeamTypes)iI).isAlive())
						{
							if (!GET_TEAM(GET_PLAYER(getOwnerINLINE()).getTeam()).isVassal((TeamTypes)iI))
							{
								...
								{
									iTempValue += GET_TEAM((TeamTypes)iI).getNumCities() * 2;
								}
							}
						}
					}
				...
				if (kBuilding.getPopulationgrowthratepercentage() != 0)
				{
					...
					if ( iFoodPerTurn > 0 )
					{
						if ( iCityHappy >= 0 )
						{
							iTempValue -= (std::min(3,iCityHappy+1)*iCurrentFoodToGrow)/iFoodPerTurn;
						}
					}

					iValue += (kBuilding.getPopulationgrowthratepercentage()*iTempValue)/100;
				}
				iTempValue = 0;
					...
					if (kBuilding.getGlobalBuildingClassProductionModifier(iI) != 0)
					{
						...
						if (eLoopBuilding != NO_BUILDING)
						{
							...
							if (canConstruct(eLoopBuilding))
							{
								...
								int iOriginalCost = getHurryCost(true, eLoopBuilding, false);
								int iNewCost;
								int iOwnedCities = GET_PLAYER(getOwnerINLINE()).getNumCities();
								if (iModifier > -100)
								{
									iNewCost = (iOriginalCost * (100 / (100 + iModifier)));
									iTempValue += iOriginalCost - iNewCost / 10;
									iTempValue *= iOwnedCities;
								}
								else
								{//If the modifier is less than -100, avoid it like the plague
									iTempValue -= 100;
				...
				{
					CvPlot* pLoopPlot;
					for (iI = 0; iI < NUM_CITY_PLOTS_1; iI++)
					{
						if (iI != CITY_HOME_PLOT)
						{
							pLoopPlot = plotCity(getX_INLINE(), getY_INLINE(), iI);
							if (pLoopPlot != NULL && (!pLoopPlot->isWater()) && (!pLoopPlot->isFreshWater()))
							{
...
/************************************************************************************************/
					...
					iTempValue *= GET_PLAYER(getOwnerINLINE()).getCommercePercent(COMMERCE_GOLD);
					iTempValue /= 100;
					iValue += iTempValue;
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/	
			...
			if (iPass > 0)
			{
				...
				const CvProperties* cityProperties = getProperties();
				const CvProperties* buildingProperties = kBuilding.getProperties();
				int num = buildingProperties->getNumProperties();
				for (iI = 0; iI < num; iI++)
				{
					CvPropertyInfo& kProperty = GC.getPropertyInfo((PropertyTypes)buildingProperties->getProperty(iI));

					int	iOperationalLow = kProperty.getOperationalRangeMin();
					int	iOperationalHigh = kProperty.getOperationalRangeMax();
					int iCurrentValue = cityProperties->getValueByProperty((PropertyTypes)buildingProperties->getProperty(iI));
					int	iBuildingValue = buildingProperties->getValue(iI);
					int iWouldBeValue = iCurrentValue + iBuildingValue;
					...
					iTempValue = (kProperty.getAIWeight()*(iWouldBeNormalizedValue - iCurrentNormalizedValue))/10;
					...
					switch(kProperty.getAIScaleType())
					{
						case AISCALE_CITY:
							break;
						case AISCALE_AREA:
							iTempValue *= area()->getCitiesPerPlayer(getOwnerINLINE());
							break;
						case AISCALE_PLAYER:
							iTempValue *= GET_PLAYER(getOwnerINLINE()).getNumCities();
							break;
						case AISCALE_TEAM:
							iTempValue *= GET_TEAM(getTeam()).getNumCities();
							break;
					}
...
/*                                                                                              */
/************************************************************************************************/
					iTempValue += ((kBuilding.getCommerceChange(iI) + GET_TEAM(getTeam()).getBuildingCommerceChange(eBuilding, (CommerceTypes)iI) + aiFreeSpecialistCommerce[iI]) * 4);
					/* Was:
					iTempValue += (kBuilding.getCommerceChange(iI) * 4);
					*/		
					iTempValue += (kBuilding.getObsoleteSafeCommerceChange(iI) * 4);
					iTempValue += (kBuilding.getCommerceAttacks(iI) * 2);
					iTempValue += getCommerceRate((CommerceTypes)iI) *  (kBuilding.getCommerceModifier(iI) + GET_TEAM(getTeam()).getBuildingCommerceModifier(eBuilding, (CommerceTypes)iI)) / 25;
					/* Was:
					iTempValue *= 100 + kBuilding.getCommerceModifier(iI);
					*/
					for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
					{
						iTempValue += getCommerceRate((CommerceTypes)iI) * kBuilding.getBonusCommerceModifier(iJ,iI) / 25;
					}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/	
...
/*                                                                                              */
/************************************************************************************************/
					if ( aiBaseCommerceRate[COMMERCE_GOLD] == MAX_INT )
					{
						aiBaseCommerceRate[COMMERCE_GOLD] = getBaseCommerceRate(COMMERCE_GOLD);
					}
					iTempValue = (((kBuilding.getCommerceModifier(COMMERCE_GOLD) + GET_TEAM(getTeam()).getBuildingCommerceModifier(eBuilding, COMMERCE_GOLD)) * aiBaseCommerceRate[COMMERCE_GOLD]) / 40);
					/* Was:
					iTempValue = ((kBuilding.getCommerceModifier(COMMERCE_GOLD) * getBaseCommerceRate(COMMERCE_GOLD)) / 40);
					*/
					for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
					{
						iTempValue += ((kBuilding.getBonusCommerceModifier(iJ,COMMERCE_GOLD) * aiBaseCommerceRate[COMMERCE_GOLD]) / 40);
					}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/	
...
/*                                                                                              */
/************************************************************************************************/
					iValue += ((kBuilding.getCommerceChange(COMMERCE_GOLD) + GET_TEAM(getTeam()).getBuildingCommerceChange(eBuilding, COMMERCE_GOLD)) * 4);
					iValue += (kBuilding.getCommerceAttacks(COMMERCE_GOLD) * 2);
					/* Was:
...
/*                                                                                              */
/************************************************************************************************/
					if ( aiBaseCommerceRate[COMMERCE_RESEARCH] == MAX_INT )
					{
						aiBaseCommerceRate[COMMERCE_RESEARCH] = getBaseCommerceRate(COMMERCE_RESEARCH);
					}
					iTempValue = (((kBuilding.getCommerceModifier(COMMERCE_RESEARCH) + GET_TEAM(getTeam()).getBuildingCommerceModifier(eBuilding, COMMERCE_RESEARCH)) * aiBaseCommerceRate[COMMERCE_RESEARCH]) / 40);
					/* Was:
					iTempValue = ((kBuilding.getCommerceModifier(COMMERCE_RESEARCH) * getBaseCommerceRate(COMMERCE_RESEARCH)) / 40);
					*/
					for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
					{
						iTempValue += ((kBuilding.getBonusCommerceModifier(iJ,COMMERCE_RESEARCH) * aiBaseCommerceRate[COMMERCE_RESEARCH]) / 40);
					}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/	
...
/*                                                                                              */
/************************************************************************************************/
					iValue += ((kBuilding.getCommerceChange(COMMERCE_RESEARCH) + GET_TEAM(getTeam()).getBuildingCommerceChange(eBuilding, COMMERCE_RESEARCH) + aiFreeSpecialistCommerce[COMMERCE_RESEARCH]) * 4);
					iValue += (kBuilding.getCommerceAttacks(COMMERCE_RESEARCH) * 2);
					/* Was:
					...
					*/
					...
					iTempValue *= GET_PLAYER(getOwnerINLINE()).getCommercePercent(COMMERCE_RESEARCH);
					iTempValue /= 100;
					iValue += iTempValue;
/************************************************************************************************/
/* Afforess	                     END                                                            */
...
/*                                                                                              */
/************************************************************************************************/
					iTempValue = ((kBuilding.getCommerceChange(COMMERCE_CULTURE) + GET_TEAM(getTeam()).getBuildingCommerceChange(eBuilding, COMMERCE_CULTURE) + aiFreeSpecialistCommerce[COMMERCE_CULTURE]) * 3);
					iTempValue += (kBuilding.getCommerceAttacks(COMMERCE_CULTURE) * 2);
					/* Was:
					iTempValue = (kBuilding.getCommerceChange(COMMERCE_CULTURE) * 3);
					*/
					//Obsolete Safe Culture is More Valuable
					iTempValue += (kBuilding.getObsoleteSafeCommerceChange(COMMERCE_CULTURE) * 10);
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/	
...
/*                                                                                              */
/************************************************************************************************/
					if ( aiBaseCommerceRate[COMMERCE_CULTURE] == MAX_INT )
					{
						aiBaseCommerceRate[COMMERCE_CULTURE] = getBaseCommerceRate(COMMERCE_CULTURE);
					}
					iValue += (((kBuilding.getCommerceModifier(COMMERCE_CULTURE) + GET_TEAM(getTeam()).getBuildingCommerceModifier(eBuilding, COMMERCE_CULTURE)) * aiBaseCommerceRate[COMMERCE_CULTURE]) / 15);
					
					iTempValue = 0;
					iTempValue += ((kBuilding.getYieldModifier(YIELD_COMMERCE) + GET_TEAM(getTeam()).getBuildingYieldModifier(eBuilding, YIELD_COMMERCE) + aiFreeSpecialistYield[YIELD_COMMERCE]) * getBaseYieldRate(YIELD_COMMERCE)) / 8;
					iTempValue += (kBuilding.getYieldChange(YIELD_COMMERCE) + GET_TEAM(getTeam()).getBuildingYieldChange(eBuilding, YIELD_COMMERCE)) * 8;
					for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
					{
						if (hasBonus((BonusTypes)iJ))
					...
					iTempValue *= GET_PLAYER(getOwnerINLINE()).getCommercePercent(COMMERCE_CULTURE);
					iTempValue /= 100;
					iValue += iTempValue;
					...
					*/
					for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
					{
						iValue += ((kBuilding.getBonusCommerceModifier(iJ,COMMERCE_CULTURE) * aiBaseCommerceRate[COMMERCE_CULTURE]) / 15);
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/	
					}
...
/*                                                                                              */
/************************************************************************************************/
					iTempValue = ((kBuilding.getCommerceModifier(COMMERCE_CULTURE) + GET_TEAM(getTeam()).getBuildingCommerceModifier(eBuilding, COMMERCE_CULTURE)) / 5);
					/* Was:
...
/*                                                                                              */
/************************************************************************************************/
					if ( aiBaseCommerceRate[COMMERCE_ESPIONAGE] == MAX_INT )
					{
						aiBaseCommerceRate[COMMERCE_ESPIONAGE] = getBaseCommerceRate(COMMERCE_ESPIONAGE);
					}
					iTempValue = (((kBuilding.getCommerceModifier(COMMERCE_ESPIONAGE) + GET_TEAM(getTeam()).getBuildingCommerceModifier(eBuilding, COMMERCE_ESPIONAGE)) * aiBaseCommerceRate[COMMERCE_ESPIONAGE]) / 80);
					/* Was:
					iTempValue = ((kBuilding.getCommerceModifier(COMMERCE_ESPIONAGE) * getBaseCommerceRate(COMMERCE_ESPIONAGE)) / 80);
					*/
					for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
					{
						iTempValue += ((kBuilding.getBonusCommerceModifier(iJ,COMMERCE_ESPIONAGE) * aiBaseCommerceRate[COMMERCE_ESPIONAGE]) / 80);
					}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/	
...
/************************************************************************************************/
					...
					iTempValue *= GET_PLAYER(getOwnerINLINE()).getCommercePercent(COMMERCE_ESPIONAGE);
					iTempValue /= 100;
					iValue += iTempValue;
					
					iTempValue = 0;
					iTempValue = ((kBuilding.getCommerceChange(COMMERCE_ESPIONAGE) + GET_TEAM(getTeam()).getBuildingCommerceChange(eBuilding, COMMERCE_ESPIONAGE) + aiFreeSpecialistCommerce[COMMERCE_ESPIONAGE]) * 3);
					iTempValue += (kBuilding.getCommerceAttacks(COMMERCE_ESPIONAGE) * 2);
					/* Was:
					iTempValue = (kBuilding.getCommerceChange(COMMERCE_ESPIONAGE) * 3);
					*/
					for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
...
bool CvCityAI::AI_isDefended(int iExtra, int iRange)
{
	PROFILE_FUNC();
...
bool CvCityAI::AI_chooseUnit(UnitAITypes eUnitAI, int iOdds, bool bIgnoreNotUnitAIs)
{
	...
	if (eBestUnit != NO_UNIT)
	{
		...
		{
			CvUnitInfo&	unitInfo = GC.getUnitInfo(eBestUnit);
			unitType = unitInfo.getType();

			OutputDebugString(CvString::format("AI chooses to build unit %s, based on type %s\n", unitType.c_str(), unitAIType.c_str()).c_str());
	...
	OutputDebugString(CvString::format("AI declines to build unit based on type %s\n", unitAIType.c_str()).c_str());
...
/************************************************************************************************/
	...
	WRAPPER_WRITE_ARRAY(wrapper, "CvCityAI", GC.getNumSpecialistInfos(), m_pbEmphasizeSpecialist);
...
public:
	...
	CvCityAI*	m_cachedCity;
	int			m_numBuildings;
	std::map<int,OneBuildingValueCache*>	m_buildingValues;
};

int	CvCityAI::GetBuildingValue(BuildingTypes eBuilding, int iFocusFlags, int iThreshold, bool bMaximizeFlaggedValue)
{
	int iResult = -1;
	...
	if ( canConstruct(eBuilding) )
	{
		if ( cachedBuildingValues != NULL && cachedBuildingValues->m_cachedCity != this )
		{
			FlushBuildingValueCache();
		}
		...
		if ( iResult == -1 )
		{
			OutputDebugString("Unexpected cache miss");
...
void CvCityAI::FlushBuildingValueCache(void)
{
	PROFILE_FUNC()
	...
	if ( cachedBuildingValues != NULL )
	{
		delete cachedBuildingValues;

		cachedBuildingValues = NULL;
	}
}

void CvCityAI::CalculateAllBuildingValues()
{
	PROFILE_FUNC()
	...
	for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		aiPlayerCommerceRate[iI] = kOwner.getCommerceRate((CommerceTypes)iI);
	}
	...
	for (iBuilding = 0; iBuilding < GC.getNumBuildingClassInfos(); iBuilding++)
	{
		...
		*/
		...
		/************************************************************************************************/
					...
					//if ((iFocusFlags & (BUILDINGFOCUS_GOLD | BUILDINGFOCUS_RESEARCH)) || iPass > 0)
					{
						PROFILE("CalculateAllBuildingValues.GoldAndResearch");
						int iValue = 0;
						// trade routes
						iTempValue = ((kBuilding.getTradeRoutes() * ((8 * std::max(0, (totalTradeModifier() + 100))) / 100))
										* (getPopulation() / 5 + 1));
						...
						valuesCache->Accumulate(BUILDINGFOCUSINDEX_GOLDANDRESEARCH,
												iValue);
					}
		...
		*/
		...
		/************************************************************************************************/	
					...
					//else
					{
						//if (iFocusFlags & BUILDINGFOCUS_GOLD)
						{
							PROFILE("CalculateAllBuildingValues.Gold2");
							int iValue = 0;
		/************************************************************************************************/
		/* Afforess	                  Start		 12/7/09                                                */
		/*                                                                                              */
		...
		/********************************************************************************/
						...
						//if (iFocusFlags & BUILDINGFOCUS_RESEARCH)
						{
							PROFILE("CalculateAllBuildingValues.Research");
							int iValue = 0;
		/************************************************************************************************/
		/* Afforess	                  Start		 12/7/09                                                */
		/*                                                                                              */
		...
		/************************************************************************************************/
							...
							*/
							...
							//
							//	FUTURE - should we do this for other commerce types too?  I am inclined to say no for hammers (since
							//	what you produce with them sacles with civ size, which the tech tree does not except very indirectly via
							//	progression through it).  Not sure about gold or espionage.
							if ( iTempValue > 0 )
							{
								iTempValue += 3;	//	Discounted futures
								iTempValue += 2*iTempValue*(1+aiBaseCommerceRate[COMMERCE_RESEARCH])/(1+aiPlayerCommerceRate[COMMERCE_RESEARCH]);	//	Proportion of whole
							}
		...
		/************************************************************************************************/	
							iValue += (kBuilding.getObsoleteSafeCommerceChange(COMMERCE_RESEARCH) * 4);
							valuesCache->AccumulateTo(BUILDINGFOCUSINDEX_RESEARCH, iValue,true);
						}

						//if (iFocusFlags & BUILDINGFOCUS_CULTURE)
						{
							PROFILE("CalculateAllBuildingValues.Culture");
							int iValue = 0;
		/************************************************************************************************/
		/* Afforess	                  Start		 12/7/09                                                */
		/*                                                                                              */
		...
		/************************************************************************************************/	
							...
							valuesCache->AccumulateTo(BUILDINGFOCUSINDEX_CULTURE, iValue,true);
						}
						
						//if (iFocusFlags & BUILDINGFOCUS_BIGCULTURE)
						{
							PROFILE("CalculateAllBuildingValues.BigCulture");
							int iValue = 0;
		/************************************************************************************************/
		/* Afforess	                  Start		 12/7/09                                                */
		/*                                                                                              */
		...
		/************************************************************************************************/	
							...
							valuesCache->AccumulateTo(BUILDINGFOCUSINDEX_BIGCULTURE, iValue,true);
						}
						
						//if (iFocusFlags & BUILDINGFOCUS_ESPIONAGE || (GC.getGameINLINE().isOption(GAMEOPTION_NO_ESPIONAGE) && (iFocusFlags & BUILDINGFOCUS_CULTURE)))
						{
							PROFILE("CalculateAllBuildingValues.Espionage");
							int iValue = 0;
		/************************************************************************************************/
		/* BETTER_BTS_AI_MOD                      01/09/10                                jdog5000      */
		/*                                                                                              */
		...
		/************************************************************************************************/
							...
							valuesCache->AccumulateTo(BUILDINGFOCUSINDEX_ESPIONAGE, iValue,true);
							if ( GC.getGameINLINE().isOption(GAMEOPTION_NO_ESPIONAGE) )
							{
								valuesCache->AccumulateTo(BUILDINGFOCUSINDEX_CULTURE, iValue,true);
							}
...
bool CvCityAI::AI_constructResourceFactories(int chance, int minValue)
{
	if (GC.getGameINLINE().getSorenRandNum(100, "Construct Resource Factory Chance") > chance)
	{
		return false;
	}
	...
	{
		int value = 0;
		if (canConstruct((BuildingTypes)building))
		{
			CvBuildingInfo& kBuilding = GC.getBuildingInfo((BuildingTypes)building);
			BonusTypes eFreeBonus = (BonusTypes)kBuilding.getFreeBonus();
			if (eFreeBonus != NO_BONUS)
			{
				for (int unit = 0; unit < GC.getNumUnitInfos(); unit++)
				{
					CvUnitInfo& kUnit = GC.getUnitInfo((UnitTypes)unit);
					bool enabled = kUnit.getPrereqAndBonus() == eFreeBonus;
					if (!enabled)
					{
						for (int prereqBonus = 0; prereqBonus < GC.getNUM_UNIT_PREREQ_OR_BONUSES(); prereqBonus++)
						{
							if (kUnit.getPrereqOrBonuses(prereqBonus) == eFreeBonus)
							{
								enabled = true;
							}
						}
					...
					if (enabled)
					{
						value += std::max(0, kUnit.getCombat() * kUnit.getCombat());
						value += std::max(0, kUnit.getAirCombat() * kUnit.getAirCombat() / 2);
					}
				}
			}
			value /= std::max(1, (1 + GET_PLAYER(getOwnerINLINE()).getBuildingClassCountPlusMaking((BuildingClassTypes)kBuilding.getBuildingClassType())));
		}
		if (value > bestValue)
		{
			bestValue = value;
			eBestBuilding = (BuildingTypes)building;
		}
	}

	if (bestValue > minValue && eBestBuilding != NO_BUILDING)
	{
		pushOrder(ORDER_CONSTRUCT, eBestBuilding, -1, false, false, false);
		return true;
	}
