...
/************************************************************************************************/
		default:
			// AIAndy: Assumed to be an outcome mission
			// FAssert(false);
			CvOutcomeList* pOutcomeList = pLoopUnit->getUnitInfo().getActionOutcomeList((MissionTypes)iMission);
			if (pOutcomeList)
			{
				if (pOutcomeList->isPossible(*pLoopUnit))
				{
					return true;
				}
			}
...
bool CvSelectionGroup::continueMission(int iSteps)
{
	...
	{
		if (headQueueNode->m_data.eMissionType == MISSION_MOVE_TO)
		{
	...
	if ((headQueueNode->m_data.iFlags & MOVE_AVOID_ENEMY_UNITS))
	{
		if (headQueueNode->m_data.eMissionType == MISSION_MOVE_TO)
		{
			if ( !checkMoveSafety(headQueueNode->m_data.iData1, headQueueNode->m_data.iData2) )
			{
...
bool CvSelectionGroup::checkMoveSafety(int iX, int iY)
{
	CvPlot* pDestPlot = GC.getMapINLINE().plotINLINE(iX, iY);

	if (generatePath(plot(), pDestPlot, 0))
	{
		pDestPlot = getPathEndTurnPlot();
	}

	return (getHeadUnit() == NULL) ? false : !((CvUnitAI*)getHeadUnit())->exposedToDanger(pDestPlot, 80);
}
...
bool CvSelectionGroup::groupPathTo(int iX, int iY, int iFlags)
{
	...
	else
	{
		...
		if ( (iFlags & MOVE_WITH_CAUTION) && !isHuman() && !canDefend() )
		{
			//	If the next plot we'd go to has a dnager count above a threshold
			//	consider it not safe and abort so we can reconsider
			if ( getPathEndTurnPlot()->getDangerCount(getTeam()) > 20 )
			{
				return false;
			}
		}
...
/************************************************************************************************/
	...
	if ( this != lastPathGeneratedFor || isHuman() )
	{
		bReuse = false;
	}
