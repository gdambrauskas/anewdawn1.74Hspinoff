...
//#define USE_DUMMY_UNIT_ENTITIES

// Public Functions...
#pragma warning( disable : 4355 )
CvUnit::CvUnit(bool bIsDummy) : m_Properties(this)
{
...
{
	return (g_dummyEntity == getEntity());
}
...
#ifdef USE_DUMMY_UNIT_ENTITIES
	if ( plot() != NULL && plot()->isActiveVisible(false) && isGroupHead())
	{
		//create new one
		CvDLLEntity::createUnitEntity(this);		// create and attach entity to unit
		setupGraphical();
...
void CvUnit::init(int iID, UnitTypes eUnit, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection)
{
	...
	//	constrauint checking on the unitAI can work more uniformly
	AI_init(eUnitAI);
...
void CvUnit::kill(bool bDelay, PlayerTypes ePlayer)
{
	...
	killUnconditional(bDelay, ePlayer);
}

void CvUnit::killUnconditional(bool bDelay, PlayerTypes ePlayer)
{
	PROFILE_FUNC();
...
/************************************************************************************************/
				...
				if (GC.getDefineINT("WAR_PRIZES"))
				{
					if (pPlot->isWater())
					{
						...
						if (bAdjacentLand)
						{
							if (GC.getGameINLINE().getSorenRandNum(4, "Unit Survives Drowning") == 0 )
							{
								bSurvived = true;
							}
						}
				...
				if (bSurvived)
				{
					...
					gDLL->getInterfaceIFace()->addMessage(pLoopUnit->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, NULL, MESSAGE_TYPE_MINOR_EVENT);
				}
				else
/************************************************************************************************/
/* Afforess	                     END                                                            */
...
/************************************************************************************************/
			...
			else
			{
				...
				pDefender->killUnconditional(false, NO_PLAYER);
				pDefender = NULL;
...
{
	return canMoveInto(pPlot, false, bDeclareWar, false, false, false, true);
}
...
bool CvUnit::setMADTargetPlot(int iX, int iY)
{
	...
	if(/*!(atWar(getTeam(), pCity->getTeam())) &&*/ (!isMADEnabled() || pPlot != getMADTargetPlot()))
	{
		...
		gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_NUKE_EXPLODES", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pCity->getX_INLINE(), pCity->getY_INLINE(), true, true);

		szBuffer = gDLL->getText("Someone has targetted %s1 with a nuke!", pCity->getNameKey());
		gDLL->getInterfaceIFace()->addMessage(pCity->getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_NUKE_EXPLODES", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pCity->getX_INLINE(), pCity->getY_INLINE(), true, true);
...
bool CvUnit::clearMADTargetPlot()
{
	...
	gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, NULL, MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), getX_INLINE(), getY_INLINE(), true, true);
...
/************************************************************************************************/
	...
	if(isMADEnabled())
	{
		...
		if(pCity == NULL || pCity->getOwnerINLINE() != getMADTargetPlotOwner())
		{
			...
			gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_NUKE_EXPLODES", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), getX_INLINE(), getY_INLINE(), true, true);
			return false;
		}
	}
	// < M.A.D. Nukes End   >
...
#ifdef C2C_BUILD
	if (!GC.getGameINLINE().isOption(GAMEOPTION_DIVINE_PROPHETS)&&(AI_getUnitAIType() != UNITAI_MISSIONARY))
	{
		return false;
	}
	
	if ((GC.getGameINLINE().isOption(GAMEOPTION_DIVINE_PROPHETS))&&(!GC.getGameINLINE().isOption(GAMEOPTION_PICK_RELIGION)))
	{
		if ((GET_TEAM(getTeam()).isHasTech((TechTypes)GC.getReligionInfo(eReligion).getTechPrereq())))
		{
			return true;
		}
		return false;
...
{
	CvOutcomeList* pOutcomeList = getUnitInfo().getActionOutcomeList(eMission);
	if (!pOutcomeList)
	{
		return false;
	}

	if (!pOutcomeList->isPossible(*this))
	{
		return false;
	}

	pOutcomeList->execute(*this);
...
/*                                                                                              */
/************************************************************************************************/
	return m_iBaseCombat + GET_TEAM(getTeam()).getUnitClassStrengthChange((UnitClassTypes)getUnitClassType());
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
}
...
#ifdef USE_DUMMY_UNIT_ENTITIES
	if ( pNewPlot != NULL &&
		 (pOldPlot != NULL && pOldPlot->isActiveVisible(false)) !=
		 (pNewPlot != NULL && pNewPlot->isActiveVisible(false)) )
	{
		reloadEntity();
	}
#endif
...
bool CvUnit::doDelayedDeath()
{
	...
	{
		killUnconditional(false, NO_PLAYER);
		return true;
	}
...
#define BATTLE_TURN_RECHECK 4
	...
	if ( (!kBattleDefinition.getUnit(BATTLE_UNIT_ATTACKER)->isUsingDummyEntities() && gDLL->getEntityIFace()->GetSiegeTower(kBattleDefinition.getUnit(BATTLE_UNIT_ATTACKER)->getUnitEntity())) ||
		 (!kBattleDefinition.getUnit(BATTLE_UNIT_DEFENDER)->isUsingDummyEntities() && gDLL->getEntityIFace()->GetSiegeTower(kBattleDefinition.getUnit(BATTLE_UNIT_DEFENDER)->getUnitEntity())) )
	{
		extraTime = BATTLE_TURNS_MELEE;
	}
		...
		if ((!isMADEnabled() || bForceRetarget) && getUnitInfo().getUnitAIType(UNITAI_ICBM))
		{
			...
			if(pBestCity != NULL)
			{
				...
				if (pOldTarget != pBestCity)
				{
					...
					gDLL->getInterfaceIFace()->addMessage(pBestCity->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_NUKE_EXPLODES", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pBestCity->getX_INLINE(), pBestCity->getY_INLINE(), true, true);
				}
			}
		}
	}
