...
/************************************************************************************************/
		if (!gDLL->isModularXMLLoading())
		{
			...
			{
/*
#ifdef _DEBUG  //This is only a notification for modders, not for players				
				FAssertMsg(pSource->getNumSaveGameVector() == GC.getModLoadControlVectorSize(), "Message by MrGenie, nothing to worry about, just a message that the Reloading of ALL XML's will start now which might take a while" );
#endif
*/
...
#ifdef _DEBUG  //This is only a notification for modders, not for players				
				char	szMessage[512];
				sprintf( szMessage, "This saved game needs a different set of modules\nTheir XML files will be reloaded automatically");
				gDLL->MessageBox( szMessage, "Saved Game Incompatibility");
#endif
