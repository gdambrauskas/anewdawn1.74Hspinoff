...
/* Fixed Borders AI                                                                             */
/************************************************************************************************/
	bool bFixedBorders = GC.getGameINLINE().isOption(GAMEOPTION_FIXED_BORDERS);
	if (bFixedBorders)
	{
...
*/
	if (!GET_PLAYER(getOwnerINLINE()).isOption(PLAYEROPTION_NO_EXPLORER_HEALING) || !isHuman())
	{
		if (getDamage() > 0)
		{
			if ((plot()->getFeatureType() == NO_FEATURE) || (GC.getFeatureInfo(plot()->getFeatureType()).getTurnDamage() == 0))
...
/*                                                                                              */
/************************************************************************************************/
	if (eUnitAI == UNITAI_SETTLE) return false; //Fixes a Waiting For Civilizations Bug.
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
bool CvUnitAI::AI_moveToInquisitionTarget()
{
	...
	if (pTargetCity != NULL)
	{
		if (generatePath(pTargetCity->plot(), 0, false))
		{
			if ((getX() != pTargetCity->getX()) || (getY() != pTargetCity->getY()))
			{
				getGroup()->pushMission(MISSION_MOVE_TO, pTargetCity->getX(), pTargetCity->getY(), MOVE_NO_ENEMY_TERRITORY, false, true, MISSIONAI_INQUISITION, pTargetCity->plot());
				return true;
			}
		}
	}
...
/* Barbarian AI                                                                                 */
/************************************************************************************************/
									if (isBarbarian() && pAdjacentPlot->getOwner() != NO_PLAYER)
									{
										iValue /= ((2 + GC.getGameINLINE().getPlayerRank(pAdjacentPlot->getOwner())) / 2);
									}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
/* Barbarian AI                                                                                 */
/************************************************************************************************/
									if (isBarbarian() && pLoopPlot->getOwner() != NO_PLAYER)
									{
										iValue /= ((2 + GC.getGameINLINE().getPlayerRank(pLoopPlot->getOwner())) / 2);
									}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
					...
					if (isEnemy(pLoopUnit->getTeam(), pLoopUnit->plot()))
					{
						if (pLoopUnit->getDomainType() == DOMAIN_SEA && !pLoopUnit->isInvisible(getTeam(), false))
						{
							if (pLoopUnit->canAttack())
							{
								...
								{
									iBestHostileMoves = std::max(iBestHostileMoves, pLoopUnit->getMoves() / GC.getMOVE_DENOMINATOR());									
								}
							}
						}
					}
...
/*
				if (AI_plotValid(pLoopPlot) && !(pLoopPlot->isBarbarian()))
*/
				if (AI_plotValid(pLoopPlot) && (!(pLoopPlot->isBarbarian()) || !GET_PLAYER(getOwnerINLINE()).isOption(PLAYEROPTION_AVOID_BARBARIANS)))
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
				{
...
bool CvUnitAI::AI_claimForts(int iMinValue, int iMaxPath)
{
	...
	if ((pBestPlot != NULL) && (pFortPlot != NULL))
	{
		...
		{
			FAssert(!atPlot(pBestPlot));
			getGroup()->pushMission(MISSION_MOVE_TO, pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE(), 0, false, false, MISSIONAI_CLAIM_TERRITORY, pFortPlot);
			return true;
		}
	}
		...
		if (AI_plotValid(pLoopPlot))
		{
			if (pLoopPlot->getOwnerINLINE() == NO_PLAYER)
			{
				if (pLoopPlot->getImprovementType() == NO_IMPROVEMENT)
				{
					...
					if (eBestTempBuild != NO_BUILD)
					{
						int iValue = pLoopPlot->getFoundValue(getOwnerINLINE()) * 100;
...
bool CvUnitAI::AI_hurryFood()
{
	...
	if ((pBestPlot != NULL) && (pBestHurryPlot != NULL))
	{
		...
		{
			FAssert(!atPlot(pBestPlot));
			getGroup()->pushMission(MISSION_MOVE_TO, pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE(), MOVE_NO_ENEMY_TERRITORY, false, false, MISSIONAI_HURRY_FOOD, pBestHurryPlot);
			return true;
		}
	}
...
bool CvUnitAI::AI_AutomatedPillage(int iBonusValueThreshold)
{
	...
	for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (AI_plotValid(pLoopPlot) && canPillage(pLoopPlot))
		{
			if (pLoopPlot->getOwnerINLINE() != BARBARIAN_PLAYER || !GET_PLAYER(getOwnerINLINE()).isOption(PLAYEROPTION_AVOID_BARBARIANS))
			{
				if (potentialWarAction(pLoopPlot))
				{
					if (GET_PLAYER(getOwnerINLINE()).isOption(PLAYEROPTION_IGNORE_DANGER) || !pLoopPlot->isVisibleEnemyUnit(this))
					{
						if (GET_PLAYER(getOwnerINLINE()).AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_PILLAGE, getGroup(), 1) == 0)
						{
							if (generatePath(pLoopPlot, 0, true, &iPathTurns))
...
void CvUnitAI::AI_borderPatrol()
{
	...
	bool bStayInBorders = GET_PLAYER(getOwnerINLINE()).isOption(PLAYEROPTION_BORDER_STAY_IN_BORDERS);
...
void CvUnitAI::AI_autoAirStrike()
{
	...
	if (kPlayer.isOption(PLAYEROPTION_AUTOMATED_AIR_CAN_REBASE))
	{
		// If no targets, no sense staying in risky place
		if (AI_airOffensiveCity())
		{
	...
	if (kPlayer.isOption(PLAYEROPTION_AUTOMATED_AIR_CAN_DEFEND))
	{
		int iAttackValue = kPlayer.AI_unitValue(getUnitType(), UNITAI_ATTACK_AIR, pArea);
		int iDefenseValue = kPlayer.AI_unitValue(getUnitType(), UNITAI_DEFENSE_AIR, pArea);
		if (iDefenseValue > iAttackValue)
	...
	if (kPlayer.isOption(PLAYEROPTION_AUTOMATED_AIR_CAN_REBASE) && kPlayer.isOption(PLAYEROPTION_AUTOMATED_AIR_CAN_DEFEND))
	{
		if (GC.getGameINLINE().getSorenRandNum(bDefensive ? 3 : 6, "AI Air Attack Move") == 0)
		{
			if( AI_defensiveAirStrike() )
	...
	if (canAirDefend() && kPlayer.isOption(PLAYEROPTION_AUTOMATED_AIR_CAN_DEFEND))
	{
		if( bDefensive || GC.getGameINLINE().getSorenRandNum(2, "AI Air Attack Move") == 0 )
		{
			getGroup()->pushMission(MISSION_AIRPATROL);
