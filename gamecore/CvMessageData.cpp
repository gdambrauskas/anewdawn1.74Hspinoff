...
{
	sprintf(szAddendum, "Updating Global Define %s, New Values are Int %d, Float %f, String %s", m_szDefine, m_iValue, m_fValue, m_szValue);
}
...
void CvGlobalDefineUpdate::Execute()
{
	if (!m_szDefine.empty())
	{
		if (m_iValue != -1)
		{
			char *szDefine = new char[255];
			sprintf(szDefine,"%S",m_szDefine.c_str());
			GC.setDefineINT(szDefine, m_iValue);
		}
		if (m_fValue != -1.0f)
		{
			char *szDefine = new char[255];
			sprintf(szDefine,"%S",m_szDefine.c_str());
			GC.setDefineFLOAT(szDefine, m_fValue);
		}
		if (!m_szValue.empty())
		{
			char *szDefine = new char[255];
			...
			GC.setDefineSTRING(szDefine, szValue);
		}
	}
}
