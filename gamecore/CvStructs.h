...
struct CvDarkAgePoints
{
	CvDarkAgePoints() {m_aiFromCivics = new int[GC.getNumCivicOptionInfos()];}
	virtual ~CvDarkAgePoints() {SAFE_DELETE_ARRAY(m_aiFromCivics);}
	int m_aiCurrentRanking[NUM_RANKINGS];
	int m_aiBestRanking[NUM_RANKINGS];
	int* m_aiFromCivics;
	int m_iExtra;
	float m_fOverallScoreFactor;
	int m_iTotal;
	int m_iTotalFinal;
};
/************************************************************************************************/
