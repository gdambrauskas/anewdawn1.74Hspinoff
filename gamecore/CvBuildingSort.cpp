...
int BuildingSortCommerce::getBuildingValue(CvPlayer *pPlayer, CvCity *pCity, BuildingTypes eBuilding)
{
	...
	{
		int iYieldCommerce = GC.getBuildingInfo(eBuilding).getYieldChange(YIELD_COMMERCE) + GC.getBuildingInfo(eBuilding).getYieldModifier(YIELD_COMMERCE) / 5;
		int iCommerce = GC.getBuildingInfo(eBuilding).getCommerceChange(m_eCommerce) + GC.getBuildingInfo(eBuilding).getCommerceModifier(m_eCommerce) / 5;
		iCommerce += iYieldCommerce * pPlayer->getCommercePercent(m_eCommerce);
		return iCommerce;
	}
}
...
int BuildingSortYield::getBuildingValue(CvPlayer *pPlayer, CvCity *pCity, BuildingTypes eBuilding)
{
	...
	{
		return GC.getBuildingInfo(eBuilding).getYieldChange(m_eYield) + GC.getBuildingInfo(eBuilding).getYieldModifier(m_eYield) / 5;
	}
}
...
void BuildingSortList::init(void)
{
	for (int i = 0; i < NUM_BUILDING_SORT; i++)
	{
		SAFE_DELETE(m_apBuildingSort[i]);
