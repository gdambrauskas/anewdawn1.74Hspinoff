...
CvTaggedSaveFormatWrapper::getSaveFormatWrapper(void)
{
	if ( NULL == pSingleton )
	{
...
void
CvTaggedSaveFormatWrapper::WriteObjectDelimiter(const char* name, bool bStart)
{
	PROFILE_FUNC();
...
CvTaggedSaveFormatWrapper::WriteClassMappingTable(RemappedClassType classType)
{
	...
	case REMAPPED_CLASS_TYPE_COMBATINFOS:
		...
		for(int i = 0; i < entry.numClasses; i++)
		{
			CvInfoBase& info = GC.getUnitCombatInfo((UnitCombatTypes)i);
...
void
CvTaggedSaveFormatWrapper::WriteClassMappingTables(void)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::WriteStartObject(const char* name)
{
	PROFILE_FUNC();
	...
	WriteObjectDelimiter(name, true);

	m_nestingDepth++;
}

void
CvTaggedSaveFormatWrapper::WriteEndObject(void)
{
	PROFILE_FUNC();
	...
	WriteObjectDelimiter(NULL, false);
}

//	The following methods must be used for entities that correspond to class enums
void
CvTaggedSaveFormatWrapper::WriteClassEnum(const char* name, RemappedClassType classType, int value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_CLASS_ENUM, true);
		entry.classType = classType;
		entry.value = value;
...
//	The following methods must be used for entities that correspond to class enums
void
CvTaggedSaveFormatWrapper::WriteClassEnum(const char* name, RemappedClassType classType, short value)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::WriteClassArray(const char* name, RemappedClassType classType, int count, const int values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_CLASS_INT_ARRAY, true);
		entry.classType = classType;
		entry.numInts = count;
...
//	The following are for arrays whose index is a class enum value
void
CvTaggedSaveFormatWrapper::WriteClassArray(const char* name, RemappedClassType classType, int count, const bool values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_CLASS_BOOL_ARRAY, true);
		entry.classType = classType;
...
void
CvTaggedSaveFormatWrapper::WriteClassArrayOfClassEnum(const char* name, RemappedClassType indexClassType, RemappedClassType valueClassType, int count, const int values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::WriteClassEnumArray(const char* name, RemappedClassType classType, int count, short values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_CLASS_ENUM_ARRAY, true);
		entry.classType = classType;
		entry.count = count;
...
//	Array whose values are class enum entities
void
CvTaggedSaveFormatWrapper::WriteClassEnumArray(const char* name, RemappedClassType classType, int count, int values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, char value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_CHAR, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, byte value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_BYTE, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const  char values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_CHAR_ARRAY, true);
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const  byte values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_BYTE_ARRAY, true);
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, bool value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_BOOL, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const bool values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_BOOL_ARRAY, true);
		entry.numBools = count;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, short value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_SHORT, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, unsigned short value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_UNSIGNED_SHORT, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const short values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_SHORT_ARRAY, true);
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const unsigned short values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_UNSIGNED_SHORT_ARRAY, true);
		entry.numShorts = count;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_INT, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, unsigned int value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_UNSIGNED_INT, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const int values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_INT_ARRAY, true);
		entry.numInts = count;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const unsigned int values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_UNSIGNED_INT_ARRAY, true);
		entry.numInts = count;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, long value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_LONG, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, unsigned long  value) 
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_UNSIGNED_LONG, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const long values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_LONG_ARRAY, true);
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const unsigned long values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_UNSIGNED_LONG_ARRAY, true);
		entry.numLongs = count;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, float value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_FLOAT, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const float values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_FLOAT_ARRAY, true);
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, double value)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_DOUBLE, true);
		entry.value = value;
...
void
CvTaggedSaveFormatWrapper::Write(const char* name, int count, const double values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_DOUBLE_ARRAY, true);
...
void
CvTaggedSaveFormatWrapper::WriteString(const char* name, const wchar *szName)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_WSTRING, true);
...
void
CvTaggedSaveFormatWrapper::WriteString(const char* name, const char *szName)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_STRING, true);
		m_stream->Write(sizeof(entry), (const byte*)&entry);
		m_stream->WriteString(szName);
	}
	else
...
void
CvTaggedSaveFormatWrapper::WriteString(const char* name, const std::string& szName)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_STD_STRING, true);
		m_stream->Write(sizeof(entry), (const byte*)&entry);
		m_stream->WriteString(szName);
	}
	else
...
void
CvTaggedSaveFormatWrapper::WriteString(const char* name, const std::wstring& szName)
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_STD_WSTRING, true);
		m_stream->Write(sizeof(entry), (const byte*)&entry);
		m_stream->WriteString(szName);
	}
	else
...
void
CvTaggedSaveFormatWrapper::WriteString(const char* name, int count, std::string values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_STRING_ARRAY, true);
...
void
CvTaggedSaveFormatWrapper::WriteString(const char* name, int count, std::wstring values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		entry.id = getId(name, SAVE_VALUE_TYPE_WSTRING_ARRAY, true);
		entry.numStrings = count;
...
int
CvTaggedSaveFormatWrapper::getId(const char* name, SaveValueType type, bool addToDictionary)
{
	PROFILE_FUNC();

	int id;
	CvString	normalizedName = NormalizeName(name);
	std::map<CvString,DictionaryEntry>::iterator itr = m_reverseIdDictionary.find(normalizedName);
...
void
CvTaggedSaveFormatWrapper::ReadString(const char* name, char **szName)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::ReadString(const char* name, wchar **szName)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::ReadString(const char* name, std::string& szName)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::ReadString(const char* name, std::wstring& szName)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::ReadString(const char* name, int count, std::string values[])
{
	PROFILE_FUNC();
	...
	if ( m_useTaggedFormat )
	{
		...
		if ( Expect(name, SAVE_VALUE_TYPE_STRING_ARRAY) )
		{
			...
			{
				//	Incompatible save
				HandleIncompatibleSave("Save format is not compatible");
			}
...
void
CvTaggedSaveFormatWrapper::ReadString(const char* name, int count, std::wstring values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, char *pChar)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, byte *pByte)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, char values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, byte values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, bool *pBool)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, bool values[], bool bAllowTruncation)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, short	*s)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, unsigned short	*s) 
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, short values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, unsigned short values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int* i)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, unsigned int* i)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, int values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, unsigned int values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, long* l)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, unsigned long* l)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, long values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, unsigned long values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, float* value)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, float values[])
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, double* value)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::Read(const char* name, int count, double values[])
{
	PROFILE_FUNC();
...
//	The following methods must be used for entities that correspond to class enums
void
CvTaggedSaveFormatWrapper::ReadClassEnum(const char* name, RemappedClassType classType, int* value, bool allowMissing)
{
	PROFILE_FUNC();
...
//	The following methods must be used for entities that correspond to class enums
void
CvTaggedSaveFormatWrapper::ReadClassEnum(const char* name, RemappedClassType classType, short* value, bool allowMissing)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::ReadClassArray(const char* name, RemappedClassType classType, int count, int values[], bool allowMissing, bool allowRawArray)
{
	PROFILE_FUNC();
		...
		if ( Expect(name, SAVE_VALUE_TYPE_CLASS_INT_ARRAY) )
		{
			...
			for(int i = 0; i < entry.numInts; i++)
			{
				...
				if ( info.m_id == -1 && !info.m_lookedUp )
				{
					...
					//	If some obehjcts are mising be tolerant provided their value was 0, -1, MIN_INT (assumed likely defaults
...
void
CvTaggedSaveFormatWrapper::ReadClassArray(const char* name, RemappedClassType classType, int count, bool values[], bool allowMissing, bool allowRawArray)
{
	PROFILE_FUNC();
...
//	The following are for arrays whose index is a class enum value and value is another class enum value
void
CvTaggedSaveFormatWrapper::ReadClassArrayOfClassEnum(const char* name, RemappedClassType indexClassType, RemappedClassType valueClassType, int count, int values[])
{
	PROFILE_FUNC();
...
//	The following methods must be used for entities that correspond to class enums
void
CvTaggedSaveFormatWrapper::ReadClassEnumArray(const char* name, RemappedClassType classType, int count, short values[], bool allowMissing)
{
	PROFILE_FUNC();
...
//	The following methods must be used for entities that correspond to class enums
void
CvTaggedSaveFormatWrapper::ReadClassEnumArray(const char* name, RemappedClassType classType, int count, int values[], bool allowMissing)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::ReadStartObject(const char* name)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::ReadEndObject(void)
{
	PROFILE_FUNC();
...
CvTaggedSaveFormatWrapper::Expect(const char* name, SaveValueType type)
{
	...
	if ( m_iNextElementType == SAVE_ELEMENT_ID_OBJECT_DELIMITER )
	{
		...
		{
			//	Non matching start element for a nested ovject.  This implies the version we are running
...
void
CvTaggedSaveFormatWrapper::SkipElement(void)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::ReadDictionaryElement(void)
{
	PROFILE_FUNC();
	...
	//	Re-normalize the name from teh dictionary element so as to be compatible (in most cases) across
...
void
CvTaggedSaveFormatWrapper::ReadClassMap(void)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::ReadObjectDelimiter(void)
{
	PROFILE_FUNC();
...
void
CvTaggedSaveFormatWrapper::close(void)
{
	if ( m_inUse )
	{
