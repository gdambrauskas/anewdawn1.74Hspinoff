...
/************************************************************************************************/
			int iProduction;
			if ((pPlot->getFeatureType() == (FeatureTypes)GC.getInfoTypeForString("FEATURE_FOREST")) && (GET_TEAM(getTeam()).isHasTech((TechTypes)GC.getBuildInfo(findChopBuild(pPlot->getFeatureType())).getTechPrereq())))
			{
			...
			else if ((pPlot->getFeatureType() == (FeatureTypes)GC.getInfoTypeForString("FEATURE_JUNGLE")) && (GET_TEAM(getTeam()).isHasTech((TechTypes)GC.getBuildInfo(findChopBuild(pPlot->getFeatureType())).getTechPrereq())))
			{
				iProduction = GC.getBuildInfo(findChopBuild(pPlot->getFeatureType())).getFeatureProduction(pPlot->getFeatureType());
...
/************************************************************************************************/
	...
	checkBuildings(true, false, true, true);
...
/************************************************************************************************/
	case TASK_SACK_CITY:
		//GET_PLAYER((PlayerTypes)iData1).acquireCity(this, false, true, true);
		GET_PLAYER((PlayerTypes)iData1).sack(this);
		break;
	case TASK_ENSLAVE_CITY:
		GET_PLAYER((PlayerTypes)iData1).acquireCity(this, false, true, true);
		GET_PLAYER(getOwnerINLINE()).enslave(this);
		break;
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
/*                                                                                              */
/************************************************************************************************/
bool CvCity::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreAmount) const
{
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
/************************************************************************************************/
		...
		{
			if (getPopulation() < kBuilding.getPrereqPopulation())
			{
				return false;
			}
		}
	...
	if (!bTestVisible)
	{
		...
		for (iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
		{
			if (kBuilding.isPrereqOrBuildingClass(iI))
			{
				...
				if (ePrereqBuilding != NO_BUILDING)
				{
					if (getNumActiveBuilding(ePrereqBuilding) > 0)
					{
						bValid = true;
						break;
					}
...
/*			if(getUnitCombatFreeExperience((UnitCombatTypes)(GC.getUnitInfo(eUnit).getUnitCombatType())) != 0)
			{
				...
				for(iI =0; iI < GC.getNumBuildingClassInfos(); iI++)
				{
					...
					if (eLoopBuilding != NO_BUILDING)
					{
						if( GET_PLAYER(getOwnerINLINE()).isBuildingClassRequiredToTrain(BuildingClassTypes(iI), eUnit) && getNumBuilding(eLoopBuilding) )
						{
...
/************************************************************************************************/
	if (eUnit != NO_UNIT)
	{
        if(GC.getUnitInfo(eUnit).isSpy() && GC.getDefineINT("SS_ENABLED"))
        {
            iExperience = 0;
...
/************************************************************************************************/
/* Afforess	            ProductionModifier Start		 11/25/09                                */
/*                                                                                              */
/*                                                                                              */
/************************************************************************************************/
	if (GC.getUnitInfo(eUnit).getUnitCombatType() != NO_UNITCOMBAT)
...
/*                                                                                              */
/************************************************************************************************/
		changePopulationgrowthratepercentage(kBuilding.getPopulationgrowthratepercentage() * iChange);
/************************************************************************************************/
...
/*                                                                                              */
/************************************************************************************************/
			updateCommerceRateByBuilding(eBuilding, (CommerceTypes)iI, ((GET_TEAM(getTeam()).getBuildingCommerceChange(eBuilding, (CommerceTypes)iI)) * iChange));
...
/************************************************************************************************/
		...
		if (kBuilding.getNoEntryDefenseLevel() > getMinimumDefenseLevel())
		{
			setMinimumDefenseLevel(kBuilding.getNoEntryDefenseLevel());
		}
		...
		{
			setFreshWater(iChange > 0 ? true : false);
		}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/	
...
*/
		changeSpecialistCommerce(((CommerceTypes)iI), ((GC.getSpecialistInfo(eSpecialist).getCommerceChange(iI) + (GET_PLAYER(getOwnerINLINE()).getSpecialistCommercePercentChanges(eSpecialist, (CommerceTypes)iI) / 100)) * iChange));
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
/************************************************************************************************/
		...
		iUnhappiness += std::max(0, GET_PLAYER(getOwnerINLINE()).getElectionAnger());
...
/*                                                                                              */
/************************************************************************************************/
	iHappiness += std::max(0, GC.getHandicapInfo(getHandicapType()).getHappyBonus() - (GC.isBetterRoM() ? getBonusGoodHappiness() / 2 : 0));
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
/*                                                                                              */
/************************************************************************************************/
	iHealth = GC.getHandicapInfo(getHandicapType()).getHealthBonus() - (GC.isBetterRoM() ? getBonusGoodHealth() / 2 : 0);
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
	if (iHealth > 0)
		...
		for(int iDX = -iMaxDistance; iDX <= iMaxDistance; ++iDX)
		{
			for(int iDY = -iMaxDistance; iDY <= iMaxDistance; ++iDY)
			{
				...
				if(distance > 1)
				{
					...
					if(m_aCultureDistances[iDX][iDY] != iNewValue)
					{
						m_aCultureDistances[iDX][iDY] = iNewValue;
						bHasChanged = true;
					}
				}
			}
...
int CvCity::calculateCultureDistance(int iDX, int iDY, int iMaxDistance) const
{
	...
	int iEastDist = m_aCultureDistances[iDX-1][iDY];
	...
	int iSouthDist = m_aCultureDistances[iDX][iDY-1];
	...
	int iWestDist = m_aCultureDistances[iDX+1][iDY];
	...
	int iNorthDist = m_aCultureDistances[iDX][iDY+1];
...
int CvCity::cultureDistance(int iDX, int iDY, bool bForce) const
{
	...
	{
		int cultureDistance = m_aCultureDistances[iDX][iDY];
		if(cultureDistance > 0) return cultureDistance;
		
		recalculateCultureDistances(plotDistance(0, 0, iDX, iDY));
		return m_aCultureDistances[iDX][iDY];
	}
...
/************************************************************************************************/
	...
	{
		iStrength *= std::max(0, (GC.getDefineINT("DARK_AGE_CULTURE_STRENGTH_PERCENT_DECREASE") + 100));
		iStrength /= 100;
	}
/************************************************************************************************/
/* Afforess	                     END                                                            */
...
/*                                                                                              */
/************************************************************************************************/
	else if (GET_PLAYER(getOwnerINLINE()).isDarkAge())
	{
		iModifier -= GC.getDefineINT("GOLDEN_AGE_GREAT_PEOPLE_MODIFIER");
	}
/************************************************************************************************/
...
int CvCity::getSavedMaintenanceTimes100ByBuilding(BuildingTypes eBuilding) const
{
	...
	{
		int iNewMaintenance = calculateBaseMaintenanceTimes100() * (getMaintenanceModifier() + iModifier + 100) / 100;
...
/************************************************************************************************/
	if (GC.isBetterRoM())
	{
		...
		for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; iPlayer++)
		{
			...
			{
				iMaxNumCities = std::max(kLoopPlayer.getNumCities(), iMaxNumCities);
			}
		}
		iNumCitiesPercent *= GET_PLAYER(getOwnerINLINE()).getNumCities();
		iNumCitiesPercent /= iMaxNumCities;
	}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
/************************************************************************************************/
/* Afforess  Tech Health & Happy Changes                 10/13/09                               */
/*                                                                                              */
/*                                                                                              */
/************************************************************************************************/
...
/*	int iSpecialistExtraHappy = 0;
	...
	for (int iI = 1; iI < kBuilding.getFreeSpecialist() + 1; iI++)
	{
		...
		if (eNewSpecialist == NO_SPECIALIST) break;
		CvSpecialistInfo& kSpecialist = GC.getSpecialistInfo(eNewSpecialist);
		iSpecialistExtraHappy += kSpecialist.getHappiness();
	}
...
/*	int iSpecialistExtraHealth = 0;
	...
	for (iI = 1; iI < kBuilding.getFreeSpecialist() + 1; iI++)
	{
		...
		if (eNewSpecialist == NO_SPECIALIST) break;
		CvSpecialistInfo& kSpecialist = GC.getSpecialistInfo(eNewSpecialist);
		iSpecialistExtraHealth += kSpecialist.getHealth();
	}
...
*/
	return iChange * (kSpecialist.getYieldChange(eIndex) + GET_PLAYER(getOwnerINLINE()).getSpecialistExtraYield(eSpecialist, eIndex) + (GET_PLAYER(getOwnerINLINE()).getSpecialistYieldPercentChanges(eSpecialist, eIndex) / 100));
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
}
...
/************************************************************************************************/
			...
			if (GC.getGameINLINE().isOption(GAMEOPTION_ADVANCED_ECONOMY))
			{
				...
				CvPlayer &kPlayer = GET_PLAYER(getOwnerINLINE());
				if (kOtherPlayer.getCurrency() != NO_BONUS)
				{
					if (kOtherPlayer.getCurrency() == GET_PLAYER(getOwnerINLINE()).getCurrency())
					{
						iModifier += GC.getDefineINT("SHARED_CURRENCY_TRADE_MODIFIER");
					}
				}
				for (int iI = 0; iI < GC.getNumCivicOptionInfos(); iI++)
				{
					...
					if (GET_PLAYER((PlayerTypes)iI).isAlive())
					{
						if (isRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false))
						{
							if ((getOwnerINLINE() == iI) || (GET_PLAYER((PlayerTypes)iI).getStateReligion() == eIndex) || GET_PLAYER((PlayerTypes)iI).hasHolyCity(eIndex))
							{
								...
								gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getDefineINT("EVENT_MESSAGE_TIME_LONG"), szBuffer, GC.getReligionInfo(eIndex).getSound(), MESSAGE_TYPE_MAJOR_EVENT, GC.getReligionInfo(eIndex).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), getX_INLINE(), getY_INLINE(), bArrows, bArrows);
							}
						}
					}
				}
...
void CvCity::setHasCorporation(CorporationTypes eIndex, bool bNewValue, bool bAnnounce, bool bArrows)
{
	...
	if (isHasCorporation(eIndex) != bNewValue)
	{
		...
		changeMilitaryProductionModifier(GC.getCorporationInfo(eIndex).getMilitaryProductionModifier() * iChange);
		changeFreeExperience(GC.getCorporationInfo(eIndex).getFreeXP() * iChange);
		/************************************************************************************************/
		/* Afforess	                     END                                                            */
		/************************************************************************************************/
...
/************************************************************************************************/
						...
						gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getCorporationInfo(eIndex).getSound(), MESSAGE_TYPE_MAJOR_EVENT, GC.getCorporationInfo(eIndex).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), getX_INLINE(), getY_INLINE(), bArrows, bArrows);
						}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
				...
				if (eBuilding != NO_BUILDING)
				{
					if (canConstruct(eBuilding, true, false, false, false))
					{
						...
						if (m_iLostProduction > 0)
						{
							...
							gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szMessage, "AS2D_WONDERGOLD", MESSAGE_TYPE_MINOR_EVENT, GC.getYieldInfo(YIELD_PRODUCTION).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), getX_INLINE(), getY_INLINE(), true, true);
				...
				if (GC.getBuildingInfo((BuildingTypes)iI).getProductionContinueBuildingClass() != NO_BUILDINGCLASS)
				{
					...
					if (eBuilding != NO_BUILDING)
					{
						if (canConstruct(eBuilding))
						{
							...
							//szBuffer = gDLL->getText("TXT_KEY_MISC_PROD_CONVERTED", getBuildingProduction((BuildingTypes)iI),  GC.getBuildingInfo((BuildingTypes)iI).getTextKeyWide(), GC.getBuildingInfo(eBuilding).getTextKeyWide());
					//gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_WONDERGOLD", MESSAGE_TYPE_MINOR_EVENT, GC.getYieldInfo(YIELD_PRODUCTION).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), getX_INLINE(), getY_INLINE(), true, true);
						}
					}
				}
/************************************************************************************************/
			...
			else if (GC.getGameINLINE().isOption(GAMEOPTION_RELIGION_DECAY))
			{
				...
				if (iI != GET_PLAYER(getOwnerINLINE()).getStateReligion())
				{
					if (getReligionCount() > 2)
					{
						...
						{
							iDecay = 100;
							//10% more decay for each religion above 3
							...
							iDecay /= std::max(1, getReligionInfluence((ReligionTypes)iI));
							if (pHolyCity != NULL)
							{
						...
						if (GC.getGameINLINE().getSorenRandNum(GC.getDefineINT("RELIGION_SPREAD_RAND"), "Religion Decay") < iDecay)
						{
...
/************************************************************************************************/
	...
	pStream->Read(NUM_COMMERCE_TYPES, m_aiCommerceAttacks);
	pStream->Read(NUM_COMMERCE_TYPES, m_aiMaxCommerceAttacks);
	pStream->Read(GC.getNumUnitClassInfos(), m_paiUnitClassProductionModifier);
	pStream->Read(GC.getNumBuildingClassInfos(), m_paiBuildingClassProductionModifier);
	pStream->Read(GC.getNumBonusInfos(), m_paiBonusDefenseChanges);
	pStream->Read(GC.getNumBonusInfos(), m_pabHadVicinityBonus);
	pStream->Read(&m_iCiv);
	pStream->Read(&m_iExtraYieldTurns);
	pStream->Read((int*)&m_eOccupationCultureLevel);
	pStream->Read(&m_iLineOfSight);
	pStream->Read(&m_iLandmarkAngerTimer);
	pStream->Read(&m_iInvasionChance);
	pStream->Read(&m_iInvasionTimer);
	pStream->Read(&m_bFreshWater);
	pStream->Read(&m_iAdjacentDamagePercent);
	pStream->Read(&m_iWorkableRadiusOverride);
	pStream->Read(&m_iProtectedCultureCount);
	pStream->Read(&m_iNumUnitFullHeal);
	pStream->Read(&m_iDisabledPowerTimer);
	pStream->Read(&m_iWarWearinessTimer);
	pStream->Read(GC.getNumBuildingInfos(), m_pabDisabledBuilding);
	
	if (uiFlag > 0)
		pStream->Read(&m_iMinimumDefenseLevel);
...
/************************************************************************************************/
	...
	pStream->Write(NUM_COMMERCE_TYPES, m_aiCommerceAttacks);
	pStream->Write(NUM_COMMERCE_TYPES, m_aiMaxCommerceAttacks);
	pStream->Write(GC.getNumUnitClassInfos(), m_paiUnitClassProductionModifier);
	pStream->Write(GC.getNumBuildingClassInfos(), m_paiBuildingClassProductionModifier);
	pStream->Write(GC.getNumBonusInfos(), m_paiBonusDefenseChanges);
	pStream->Write(GC.getNumBonusInfos(), m_pabHadVicinityBonus);
	pStream->Write(m_iCiv);
	pStream->Write(m_iExtraYieldTurns);
	pStream->Write(m_eOccupationCultureLevel);
	pStream->Write(m_iLineOfSight);
	pStream->Write(m_iLandmarkAngerTimer);
	pStream->Write(m_iInvasionChance);
	pStream->Write(m_iInvasionTimer);
	pStream->Write(m_bFreshWater);
	pStream->Write(m_iAdjacentDamagePercent);
	pStream->Write(m_iWorkableRadiusOverride);
	pStream->Write(m_iProtectedCultureCount);
	pStream->Write(m_iNumUnitFullHeal);
	pStream->Write(m_iDisabledPowerTimer);
	pStream->Write(m_iWarWearinessTimer);
	pStream->Write(GC.getNumBuildingInfos(), m_pabDisabledBuilding);
	
	pStream->Write(m_iMinimumDefenseLevel);
	
	pStream->Write(m_aBuildingCommerceModifier.size());
	...
	pStream->Write(m_aBuildingYieldModifier.size());
...
bool CvCity::isInquisitionConditions()
{
	...
	if (isHasReligion(eStateReligion))
	{
		for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
		{
			...
			{
				if ( !(isHolyCity(ReligionTypes(iI)))
				|| (isHolyCity(ReligionTypes(iI)) && GC.getDefineINT("OC_RESPAWN_HOLY_CITIES")) )
				{
					bValid = true;
					break;
				}
...
*/
void CvCity::changePopulationgrowthratepercentage(int iChange)
{
	if (0 != iChange)
	{
		m_iPopulationgrowthratepercentage += iChange;
	}
}
...
void CvCity::doPromotion()
{
    ...
    for (iI = 0; iI < GC.getNumBuildingInfos(); iI++)
    {
        if (getNumBuilding((BuildingTypes)iI) > 0)
        {
            ...
            {
                if (GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion() != NO_PROMOTION || GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion_2() != NO_PROMOTION || GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion_3() != NO_PROMOTION)
                {
                    pUnitNode = plot()->headUnitNode();
                    while (pUnitNode != NULL)
                    {
                        ...
                        if ((pLoopUnit->getUnitCombatType() != NO_UNITCOMBAT) && GC.getPromotionInfo((PromotionTypes)GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion()).getUnitCombat(pLoopUnit->getUnitCombatType()))
                        {
						 ...
						 if ((pLoopUnit->getUnitCombatType() != NO_UNITCOMBAT) && GC.getPromotionInfo((PromotionTypes)GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion_2()).getUnitCombat(pLoopUnit->getUnitCombatType()))
                        {
                            pLoopUnit->setHasPromotion((PromotionTypes)GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion_2(), true);
                        }
						 if ((pLoopUnit->getUnitCombatType() != NO_UNITCOMBAT) && GC.getPromotionInfo((PromotionTypes)GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion_3()).getUnitCombat(pLoopUnit->getUnitCombatType()))
                        {
                            pLoopUnit->setHasPromotion((PromotionTypes)GC.getBuildingInfo((BuildingTypes)iI).getFreePromotion_3(), true);
                        }
                    }
                }
            }
...
bool CvCity::isValidTerrainForBuildings(BuildingTypes eBuilding) const
{
	...
	{
		if (GC.getBuildingInfo(eBuilding).isPrereqOrTerrain(iI))
		{
		...
		if (GC.getBuildingInfo(eBuilding).isPrereqAndTerrain(iI))
		{
			if (iI == iTerrainPeak)
				bPeak2 = true;
			else if (iI == iTerrainHill)
			...
			if (bValidAndTerrain)
			{
				bValidAndTerrain = false;
				HasAndTerrain++;
			}
		}
	}
	if (RequiresAndTerrain != HasAndTerrain)
	{
		return false;
	}
	...
	{
		if (GC.getBuildingInfo(eBuilding).isPrereqOrImprovement(iI))
		{
	...
	{
		if (GC.getBuildingInfo(eBuilding).isPrereqOrFeature(iI))
		{
...
void CvCity::setFreshWater(bool bNewVal)
{
	if (bNewVal != m_bFreshWater)
	{
		m_bFreshWater = bNewVal;
	}
}
		...
		for (int iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
		{
			...
			if (eLoopBuilding != NO_BUILDING)
			{
				if (GC.getBuildingInfo(eLoopBuilding).isReplaceBuildingClass(kBuilding.getBuildingClassType()))
				{
					...
					{
						iExtraRate -= GC.getBuildingInfo(eLoopBuilding).getDefenseModifier();
						for (int iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
						{
							if (hasBonus((BonusTypes)iJ))
							{
			...
			if (!GET_TEAM(getTeam()).isObsoleteBuilding(eLoopBuilding))
			{
				if (getNumBuilding(eLoopBuilding) > 0 || isDisabledBuilding(eLoopBuilding))
				{
					if (bVicinityBonus)
					{
						...
						{
							if (GC.getBuildingInfo(eLoopBuilding).getPrereqOrVicinityBonuses(iJ) != NO_BONUS)
							{
								bNeedsVicinityBonus = true;
								if (hasVicinityBonus((BonusTypes)GC.getBuildingInfo(eLoopBuilding).getPrereqOrVicinityBonuses(iJ)))
								{
									bHasVicinityBonus = true;
								}
							}
						}
						if (GC.getBuildingInfo(eLoopBuilding).getPrereqVicinityBonus() != NO_BONUS)
						{
							bNeedsVicinityBonus = true;
							//AND bonus overrides OR bonuses
							bHasVicinityBonus = false;
							if (hasVicinityBonus((BonusTypes)GC.getBuildingInfo(eLoopBuilding).getPrereqVicinityBonus()))
							{
								bHasVicinityBonus = true;
							}
						}
					...
					{
						//Not at war
						if (GC.getBuildingInfo(eLoopBuilding).isPrereqWar())
						{
					...
					if (bCivics)
					{
						if (GC.getBuildingInfo(eLoopBuilding).isRequiresActiveCivics())
						{
					...
					CvWString szBuffer;
					if (bRestoreBuildings && isDisabledBuilding(eLoopBuilding))
					{
						setDisabledBuilding(eLoopBuilding, false);
						if (!GET_PLAYER(getOwnerINLINE()).isModderOption(MODDEROPTION_IGNORE_DISABLED_ALERTS))
						{
							szBuffer = gDLL->getText("TXT_KEY_CITY_RESTORED_BUILDINGS", getNameKey(), GC.getBuildingInfo(eLoopBuilding).getDescription(), GC.getBuildingInfo(eLoopBuilding).getDescription());
							gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, NULL, MESSAGE_TYPE_MINOR_EVENT, GC.getBuildingInfo(eLoopBuilding).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), getX_INLINE(), getY_INLINE(), true, true);
						}
					}
					else if (bObsoleteBuildings && !isDisabledBuilding(eLoopBuilding))
					{
						setDisabledBuilding(eLoopBuilding, true);
						if (bVicinityBonus || bWar)
							szBuffer = gDLL->getText("TXT_KEY_CITY_REMOVED_BUILDINGS_RESOURCES", GC.getBuildingInfo(eLoopBuilding).getDescription(), getNameKey(), GC.getBuildingInfo(eLoopBuilding).getDescription());
						else if (bCivics)
							szBuffer = gDLL->getText("TXT_KEY_CITY_REMOVED_BUILDINGS_CIVICS", GC.getBuildingInfo(eLoopBuilding).getDescription(), getNameKey(), GC.getBuildingInfo(eLoopBuilding).getDescription());
						else if (bPower)
							szBuffer = gDLL->getText("TXT_KEY_CITY_REMOVED_BUILDINGS_POWER", GC.getBuildingInfo(eLoopBuilding).getDescription(), getNameKey());
						if (!GET_PLAYER(getOwnerINLINE()).isModderOption(MODDEROPTION_IGNORE_DISABLED_ALERTS))
						{
							gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, NULL, MESSAGE_TYPE_MINOR_EVENT, GC.getBuildingInfo(eLoopBuilding).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"), getX_INLINE(), getY_INLINE(), true, true);
						}
					}
				}
			}
		...
		if (iChange != 0)
		{
			for (int iJ = 0; iJ < GC.getNumBuildingInfos(); iJ++)
			{
				if (getNumRealBuilding((BuildingTypes)iJ) > 0)
				{
					...
					{
						updateYieldRate((BuildingTypes)iJ, (YieldTypes)iK, ((getBuildingYieldChange((BuildingClassTypes)(GC.getBuildingInfo((BuildingTypes)iJ).getBuildingClassType()), (YieldTypes)iK)) + (GC.getBuildingInfo((BuildingTypes)iJ).getVicinityBonusYieldChanges((BonusTypes)iI, iK) * iChange)));
					}
				}
			}
		}
...
void CvCity::doInvasion()
{
		...
		if (bTestInvasion)
		{
			if (GC.getGameINLINE().getSorenRandNum(100, "Enemy Invades City Chance") < getInvasionChance())
			{
				...
				if (GET_PLAYER(ePlayer).isHuman())
				{
					...
					gDLL->getInterfaceIFace()->addMessage(ePlayer, false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_BUILD_BARRACKS", MESSAGE_TYPE_INFO, ARTFILEMGR.getInterfaceArtInfo("WORLDBUILDER_CITY_EDIT")->getPath(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), getX_INLINE(), getY_INLINE(), true, true);
				}
			}
		}
	}
	else
	{
		...
		if (!isInvaded())
		{
			...
			gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_BUILD_BARRACKS", MESSAGE_TYPE_INFO, ARTFILEMGR.getInterfaceArtInfo("WORLDBUILDER_CITY_EDIT")->getPath(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), getX_INLINE(), getY_INLINE(), true, true);
		}
	}
}
...
bool CvCity::isZoneOfControl() const
{
	for (int iI = 0; iI < GC.getNumBuildingInfos(); iI++)
	{
		if (getNumActiveBuilding((BuildingTypes)iI) > 0)
		{
			if (GC.getBuildingInfo((BuildingTypes)iI).isZoneOfControl())
			{
				return true;
			}
		}
							...
							if (GET_TEAM(getTeam()).isAtWar(pLoopUnit->getTeam()))
							{
								if (pLoopUnit->baseCombatStr())
								{
									...
									if (!abInformPlayer[pLoopUnit->getOwnerINLINE()])
									{
										...
										szBuffer = gDLL->getText("TXT_KEY_MISC_UNITS_DAMAGED", getNameKey());
										gDLL->getInterfaceIFace()->addMessage(pLoopUnit->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, NULL, MESSAGE_TYPE_INFO, pLoopUnit->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pLoopUnit->getX_INLINE(), pLoopUnit->getY_INLINE(), true, true);
									}
								}
							}
						}
							...
							if (iI != iJ)
							{
								if (GC.getGameINLINE().isCompetingCorporation((CorporationTypes)iJ, (CorporationTypes)iI))
								{
									if (isActiveCorporation((CorporationTypes)iJ))
									{
										...
										gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getCorporationInfo((CorporationTypes)iJ).getSound(), MESSAGE_TYPE_MAJOR_EVENT, GC.getCorporationInfo((CorporationTypes)iJ).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), getX_INLINE(), getY_INLINE(), false, false);
									}
								}
							}
						}
...
void CvCity::calculateExtraTradeRouteProfit(int iExtra, int* &aiTradeYields) const
{
	...
	paiBestValue = new int[GC.getDefineINT("MAX_TRADE_ROUTES")];
	paTradeCities = new IDInfo[GC.getDefineINT("MAX_TRADE_ROUTES")];
	...
	if (!isDisorder() && !isPlundered())
	{
		iTradeRoutes = std::min(getTradeRoutes() + iExtra, GC.getDefineINT("MAX_TRADE_ROUTES"));
