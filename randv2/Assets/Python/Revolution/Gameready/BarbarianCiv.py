...
class BarbarianCiv :
	...
	def __init__(self, customEM, RevOpt ) :
		...
		# phungus420 New World Logic
		self.iNewWorldErasBehind = self.RevOpt.getNewWorldErasBehind()
		self.iNewWorldBonuses = self.RevOpt.getNewWorldBonuses()
...
##********************************
	...
	def onEndPlayerTurn( self, argsList ) :
		...
		if( iNextPlayer >= 0 and iNextPlayer <= gc.getMAX_PLAYERS()):
	...
	def getScenario( self, pArea, bVerbose = True ) :
		...
		if( pArea == None ) :
			...
			for iPlayer in range(0,gc.getMAX_CIV_PLAYERS()) :
				if( gc.getPlayer(iPlayer).isAlive() and gc.getPlayer(iPlayer).isHuman()
				and (game.getActivePlayer() == iPlayer) ):
					game.setForcedAIAutoPlay(iPlayer, 1, true)
...
#						break
	...
	def giveTechs( self, newPlayerIdx, closeTeams, bNewWorldScenario, bBuilderBonus = 0, militaryStyle = None, bVerbose = True ) :
		...
		for techID in range(0,gc.getNumTechInfos()) :
			...
			#phungus420 New World Logic
			if( (not bNewWorldScenario) or (self.iNewWorldPolicy == 0) or (self.iNewWorldBonuses > 0) ) :
...
# Rise of Mankind 2.7
	...
	def getUnitsForPlayer( self, iPlayer, bVerbose = False, bSilent = False ) :
		...
		iWorker = newPlayer.getBestUnitType(UnitAITypes.UNITAI_WORKER)
		iSettler = newPlayer.getBestUnitType(UnitAITypes.UNITAI_SETTLE)
		iScout = newPlayer.getBestUnitType(UnitAITypes.UNITAI_EXPLORE)
		# phungus420 end
...
# Rise of Mankind 2.6 end
	...
	def createMinorCiv( self, cityList, bVerbose = True ) :
		...
		#phungus420 New World Logic
		if( bNewWorldScenario and not bForeignCivCities):
			if( (game.getHighestEra() - game.getStartEra()) < self.iNewWorldErasBehind ):
				if( self.LOG_DEBUG and bVerbose ) : CvUtil.pyPrint("  BC - Cancel settle due to Game Era")
				return
		#phungus end
...
# Rise of Mankind 2.6 end
		...
		if( self.LOG_BARB_SETTLE ) :
			...
			bLaunchPopup = self.bBarbSettlePopup and (not self.blockPopupInAuto or game.getAIAutoPlay(game.getActivePlayer()) == 0)
			...
			if( bLaunchPopup and gc.getPlayer(iPlayer).isHuman() and game.getActivePlayer() == iPlayer) :
				popup = PyPopup.PyPopup(RevDefs.barbSettlePopup,contextType = EventContextTypes.EVENTCONTEXT_ALL)
				bodStr = bodStr1 + "\n\n" + bodStr2
	...
	def checkMinorCivs( self, iPlayer, bVerbose = True ) :
		# Check minor civs for accomplishments which warrant settling into full civif( (iPlayer < 0) or (iPlayer > gc.getMAX_PLAYERS())):
...
# Rise of Mankind 2.61 end
		...
		for i,city in enumerate(cityList) :
			...
			# Units
			if( city.getPopulation() > 3 and (not bNewWorldScenario or self.iNewWorldPolicy < 2) ) :
			...
			if( style == "Military" ) :
				...
				if( city.getPopulation() > 4 and ((not bNewWorldScenario) or (self.iNewWorldPolicy == 0) or (self.iNewWorldBonuses > 1)) ) :
...
# Rise of Mankind 2.6 end
		...
		if( self.LOG_BARB_SETTLE ) :
			...
			if( bLaunchPopup and gc.getPlayer(newPlayer.getID()).isHuman() and game.getActivePlayer() == newPlayer.getID()) :
				popup = PyPopup.PyPopup(RevDefs.barbSettlePopup,contextType = EventContextTypes.EVENTCONTEXT_ALL)
				bodStr = bodStr1 + "\n\n" + bodStr2
				if( self.offerControl ) :
					bodStr += localText.getText("TXT_KEY_BARBCIV_SPREAD_OPT", ())%(leadName)
				...
				if( newPlayer.getID() == game.getActivePlayer() ) :
					...
					if( self.offerControl ) :
						...
						self.newPlayerIdx = newPlayer.getID()
						if( self.bCancelAutoForOffer ) :
							game.setForcedAIAutoPlay(game.getActivePlayer(), 1, true )
		...
		if( False ) :
			...
			game.setForcedAIAutoPlay(iPrevHuman, 1, true)
