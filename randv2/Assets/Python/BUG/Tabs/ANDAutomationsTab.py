...
class ANDAutomationsTab(BugOptionsTab.BugOptionsTab):
	...
	def normalizeBuildNames(self):
		...
		for iI in range(gc.getNumBuildInfos()):
			#Strips the <link="IMPROVEMENT_FOOBAR"</link> from the description
			szDescription = str( gc.getBuildInfo(iI).getDescription())
			szNewDescription = ""
			...
			if (iStartIndex > -1):
				szNewDescription = szDescription[0:iStartIndex] + szDescription[iEndIndex:len(szDescription)-7]
			else:
	...
	def create(self, screen):
		...
		for iI in range(gc.getNumBuildInfos()):
			if (team.isHasTech(gc.getBuildInfo(iI).getTechPrereq())):
				...
				control = str(player.getNameKey() + gc.getBuildInfo(iI).getDescription() + "Check")
