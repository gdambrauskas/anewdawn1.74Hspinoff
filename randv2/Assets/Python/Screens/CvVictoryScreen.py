...
# Prevents division by zero at beginning of game; picky, I know.
		...
		ownedLand = 0
		for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
			ownedLand += gc.getTeam(iLoopTeam).getTotalLand()
		...
		ourCulture = gc.getTeam(activePlayer.getTeam().getID()).countTotalCulture()
		
		for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
			worldCulture += gc.getTeam(iLoopTeam).countTotalCulture()
			if (gc.getTeam(iLoopTeam).isAlive() and not gc.getTeam(iLoopTeam).isMinorCiv() and not gc.getTeam(iLoopTeam).isBarbarian()):
				if (iLoopTeam != iActiveTeam and (activePlayer.getTeam().isHasMet(iLoopTeam) or gc.getGame().isDebugMode())):
...
#################################################################################################
	# Get Best Team
		...
		iTVScore = 0		
		for iLoopPlayer in range(gc.getMAX_CIV_PLAYERS()):
			if ((gc.getPlayer(iLoopPlayer).isAlive()) and (iLoopPlayer != activePlayer.getID())):
	...
	# Power History
		...
		# if (worldPower < 0): #changed to 1 because of a divide by zero error further down
		if (worldPower < 1):
			worldPower = 1;
		...
		for iLoopVC in range(gc.getNumVictoryInfos()):
			...
			if (victory.isTotalVictory()):
				if gc.getGame().isVictoryValid(iLoopVC):
					...
					if ((gc.getGame().getMaxTurns() > gc.getGame().getElapsedGameTurns() and (gc.getGame().getMaxTurns()/8) < gc.getGame().getElapsedGameTurns()) or gc.getGame().cheatCodesEnabled()):
					...
					elif ((gc.getGame().getMaxTurns()/8) > gc.getGame().getElapsedGameTurns()):						
						iRow = screen.appendTableRow(szTable)
						iRow = screen.appendTableRow(szTable)
				...
				# Headings
					...
					screen.setTableText(szTable, 3, iRow, u"<font=2b>" + "(Rival Civilization)" + u"</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
				...
				# Population
					...
					if (iBestTeam != -1) and (gc.getTeam(iBestTeam).hasMetHuman()):
						...
						screen.setTableText(szTable, 3, iRow, "Unknown", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
				...
				# WonderScores
					...
					screen.setTableText(szTable, 1, iRow, u"%i  (%i of %i built)" %(iTeamWonderScore, iTotalWorldWondersBuilt, iTotalWorldWondersPossible), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
				...
				# Religion
					...
					if (iOurReligion != -1):
						...
						iRow = screen.appendTableRow(szTable)
						screen.setTableText(szTable, 1, iRow, u"No Holy City", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					...
					if (iBestTeam != -1) and (gc.getTeam(iBestTeam).hasMetHuman()):
						...
						if (iReligionFound == 0):
							iRefRow += 1
							screen.setTableText(szTable, 3, iRefRow, u"No Holy City", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					else:
						screen.setTableText(szTable, 3, iRow, "Unknown", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					...
					ourBestCities = self.getListCultureCitiesTeam(activePlayer.getTeam().getID())[0:(iPlayerLegendaryCities + 3)]
				...
				# The rest of the scores
					screen.setTableText(szTable, 0, iRow, u"<font=4b>" + localText.getText("Other Civ's Scores",()) + u"</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					iRow = screen.appendTableRow(szTable)
					for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
...
#Sevo--VCM
	def getTeamWonderScore(self, teamID):
		...
		# iLoopTeam is a misnomer; should be iLoopPlayer.  Bah.
		for iLoopTeam in range(gc.getMAX_CIV_PLAYERS()):
			if (gc.getPlayer(iLoopTeam).isAlive() and not gc.getPlayer(iLoopTeam).isMinorCiv() and not gc.getPlayer(iLoopTeam).isBarbarian()):
				if (gc.getPlayer(iLoopTeam).getTeam() == teamID):
					teamWonderScore += gc.getPlayer(iLoopTeam).getSevoWondersScore(0)
	...
	def getTeamTotalCulture(self, iTeam):
		...
		for iLoopTeam in range(gc.getMAX_CIV_PLAYERS()):
			if (gc.getPlayer(iLoopTeam).isAlive() and not gc.getPlayer(iLoopTeam).isMinorCiv() and not gc.getPlayer(iLoopTeam).isBarbarian()):
				if (gc.getPlayer(iLoopTeam).getTeam() == teamID):
					teamCulture += gc.getPlayer(iLoopTeam).countTotalCulture()
