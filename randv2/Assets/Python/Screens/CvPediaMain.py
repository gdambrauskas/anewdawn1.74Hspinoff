...
##### DCM START
			CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_DCM	: self.placeDCMConcepts,
##### DCM END
##### Afforess START
			CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_AND	: self.placeANDConcepts,
##### Afforess END
			CivilopediaPageTypes.CIVILOPEDIA_PAGE_HINTS	: self.placeHints,
			}
		
	def getScreen(self):
		return CyGInterfaceScreen(self.PEDIA_MAIN_SCREEN_NAME, CvScreenEnums.PEDIA_CONCEPT_AND)
...
##### DCM START
		self.szCategoryConceptDCM = localText.getText("TXT_KEY_PEDIA_CATEGORY_CONCEPT_DCM", ())
##### DCM END
##### Afforess START
		self.szCategoryConceptAND = localText.getText("TXT_KEY_PEDIA_CATEGORY_CONCEPT_AND", ())
##### Afforess END
...
##### DCM START
                                                                self.szCategoryConceptDCM,
##### DCM END
##### Afforess START
                                                                self.szCategoryConceptAND,
##### Afforess END
...
##### DCM START
	def placeDCMConcepts(self):
		...
		# Create and place a tech pane									
		list = self.getSortedList( gc.getNumDCMConceptInfos(), gc.getDCMConceptInfo )
		...
		for item in list:
			...
			if iRow >= iNumRows:
				iNumRows += 1
				screen.appendTableRow(tableName)
			screen.setTableText(tableName, iColumn, iRow, u"<font=3>" + item[0] + u"</font>", gc.getDCMConceptInfo(item[1]).getButton(), WidgetTypes.WIDGET_PEDIA_DESCRIPTION, CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_DCM, item[1], CvUtil.FONT_LEFT_JUSTIFY)
			iCounter += 1
##### DCM END
...
##### Afforess START
	def placeANDConcepts(self):
		...
		# Create and place a tech pane									
		list = self.getSortedList( gc.getNumANDConceptInfos(), gc.getANDConceptInfo )
		...
		for item in list:
			...
			if iRow >= iNumRows:
				iNumRows += 1
				screen.appendTableRow(tableName)
			screen.setTableText(tableName, iColumn, iRow, u"<font=3>" + item[0] + u"</font>", gc.getANDConceptInfo(item[1]).getButton(), WidgetTypes.WIDGET_PEDIA_DESCRIPTION, CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_AND, item[1], CvUtil.FONT_LEFT_JUSTIFY)
			iCounter += 1
##### Afforess END
	...
	def pediaShow(self):
		if (not self.pediaHistory):
			self.pediaHistory.append((CvScreenEnums.PEDIA_CONCEPT_AND, 0))
...
##### DCM START
		if (szLink == "PEDIA_MAIN_CONCEPT_DCM"):
			return self.pediaJump(CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_DCM), True)
##### DCM END
##### Afforess START
		if (szLink == "PEDIA_MAIN_CONCEPT_AND"):
			return self.pediaJump(CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_AND), True)
##### Afforess END
...
##### Afforess START
		for i in range(gc.getNumNewConceptInfos()):
			if (gc.getNewConceptInfo(i).isMatchForLink(szLink, False)):
				iEntryId = self.pediaHistorical.getIdFromEntryInfo(CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_AND, i)
				return self.pediaJump(CvScreenEnums.PEDIA_HISTORY, iEntryId, True)
##### Afforess END
