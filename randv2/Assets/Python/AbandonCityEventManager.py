...
class AbandonCityEventManager:
	...
	def __eventAbandonCityDestroyBuildingBegin(self, argsList):
		...
		for iBuilding in range(gc.getNumBuildingClassInfos()):
			iType = gc.getBuildingClassInfo(iBuilding).getDefaultBuildingIndex()
			#BugUtil.debug("for loop num %d, building %s", iBuilding, gc.getBuildingInfo(iBuilding).getDescription())
	...
	def __eventAbandonCityDestroyBuildingApply(self, playerID, userData, popupReturn):
		...
		if (popupReturn.getButtonClicked() == 0):
			if (popupReturn.getSelectedPullDownValue(0) == g_iAbandonTrigger ):
				...
				#~ BugUtil.debug("gatherer (%d)  tribe (%d).",  iWorker, iSettler)
				if ( gc.getTeam(team).isHasTech(gc.getInfoTypeForString("TECH_SEDENTARY_LIFESTYLE")) == true ):
					iWorker = gc.getInfoTypeForString("UNIT_WORKER")
					iSettler = gc.getInfoTypeForString("UNIT_SETTLER")
					#~ BugUtil.debug("worker (%d) settler (%d).",  iWorker, iSettler)
				...
				#~ BugUtil.debug("unique worker (%d) settler (%d).",  iUniqueWorkerUnit, iUniqueSettlerUnit)
