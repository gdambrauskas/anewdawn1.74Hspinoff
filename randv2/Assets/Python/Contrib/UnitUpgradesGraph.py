...
promotionsSplitIncoming = ["PROMOTION_BLITZ", "PROMOTION_KAMIKAZE", "PROMOTION_CHARGE", "PROMOTION_SPEED", "PROMOTION_MEDIC1", "PROMOTION_MARCH", "PROMOTION_AMBUSH", "PROMOTION_FORMATION", "PROMOTION_COVER", "PROMOTION_PINCH", "PROMOTION_HEAL", "PROMOTION_SHOCK", "PROMOTION_SENTRY", "PROMOTION_URBAN_TACTICS1", "PROMOTION_SENSORS", "PROMOTION_NANOIDS"]
...
#Afforess - Create Building Upgrade Tree
BuildingExceptionList = []
BuildingSplitIncoming = []
BuildingSplitOutgoing = []
BuildingsButtonSize = 64
BuildingHorizontalMargin = 20
BuildingVerticalMargin = 20
BuildingHorizontalSpacing = 75
BuildingVerticalSpacing = 5
...
# Rise of Mankind start
		...
		self.promotionsSplitOutgoing = promotionsSplitOutgoing
		self.bUnit = True
# Rise of Mankind end
		#Afforess
		self.BuildingSplitIncoming = BuildingSplitIncoming
		self.BuildingSplitOutgoing = BuildingSplitOutgoing
...
# Rise of Mankind start
	def getPromotionType(self, e):
		"Returns the type of the promotions with the specified id"
		return gc.getPromotionInfo(e).getType()	
# Rise of Mankind end
	
	def getBuildingType(self, e):
		"Returns the type of the promotions with the specified id"
		return gc.getBuildingInfo(e).getType()	
	...
	def getGraph(self):
		...
		for k in range(self.getNumberOfUnits()):
			...
			#Afforess
			if (self.bUnit == True):
				if (gc.getUnitInfo(unit).getPrereqCorporation() != -1):
					continue
				if (self.getUnitType(unit) not in self.exceptionList):
					graph[unit] = Node()
			elif (self.bBuilding == True):
				if (gc.getBuildingInfo(unit) not in self.exceptionList):
					graph[unit] = Node()
			else:
				if (self.getPromotionType(unit) not in self.exceptionList):
					graph[unit] = Node()
			#Afforess
...
# Rise of Mankind 2.7 - fix for bug that prevents showing unit upgrade page due to complexity of RoM upgrade paths
...
class PromotionsGraph(UnitUpgradesGraph):
	...
	def __init__(self, pediaScreen):
		self.mGraphs = []
		self.horizontalMargin = BuildingHorizontalMargin
		self.verticalMargin = BuildingVerticalMargin
		self.horizontalSpacing = BuildingHorizontalSpacing
		self.verticalSpacing = BuildingVerticalSpacing
		self.pediaScreen = pediaScreen
		self.upgradesList = pediaScreen.UPGRADES_GRAPH_ID
		...
		self.splitOutgoing = unitSplitOutgoing
		# Rise of Mankind start
		#self.promotionsSplitIncoming = ["PROMOTION_BLITZ"]
		self.promotionsSplitIncoming = promotionsSplitIncoming
		self.promotionsSplitOutgoing = promotionsSplitOutgoing
		self.bUnit = False
		self.bBuilding = False
...
# Rise of Mankind start
	def getPromotionType(self, e):
		"Returns the type of the promotions with the specified id"
		promo = gc.getPromotionInfo(e).getType()
		# CvUtil.pyPrint("promo:" + promo + "\n")
		return gc.getPromotionInfo(e).getType()
# Rise of Mankind end
...
class BuildingsGraph(UnitUpgradesGraph):
	...
	def __init__(self, pediaScreen):
		...
		self.pediaScreen = pediaScreen
		self.upgradesList = pediaScreen.UPGRADES_GRAPH_ID
		self.buttonSize = BuildingsButtonSize
		self.exceptionList = BuildingExceptionList
		self.splitIncoming = unitSplitIncoming
		self.splitOutgoing = unitSplitOutgoing
		self.BuildingSplitIncoming = BuildingSplitIncoming
		self.BuildingSplitOutgoing = BuildingSplitOutgoing
	...
	def getUnitNumber(self, k):
		"Gets the id for the kth unit for the current active player"
		if (self.getActivePlayer() == -1):
			result = gc.getBuildingClassInfo(k).getDefaultBuildingIndex()
		else:
			result = gc.getCivilizationInfo(gc.getGame().getActiveCivilizationType()).getCivilizationBuildings(k)
		return result

	def getBuildingType(self, e):
		"Returns the type of the Building with the specified id"
		return gc.getBuildingInfo(e).getType()
