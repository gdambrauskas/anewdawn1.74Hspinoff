...
class SevoPediaImprovement:
	...
	def __init__(self, main):
		...
		self.H_EFFECTS = 112#self.top.B_PEDIA_PAGE - self.Y_EFFECTS Roamty
		...
		self.W_BONUS_YIELDS_PANE = 250#self.top.R_PEDIA_PAGE - self.X_BONUS_YIELDS_PANE Roamty
		self.H_BONUS_YIELDS_PANE = 242#self.top.B_PEDIA_PAGE - self.Y_BONUS_YIELDS_PANE Roamty


		self.X_HISTORY_PANE = 754
		self.Y_HISTORY_PANE = 536
		self.W_HISTORY_PANE = 250
	...
	def placeYield(self):
		...
		for iTrait in range(gc.getNumTraitInfos()):
			for k in range(YieldTypes.NUM_YIELD_TYPES):
				szYield = u""
				iYieldChange = gc.getImprovementInfo(self.iImprovement).getTraitYieldChanges(iTrait, k)
				if (iYieldChange != 0):
					szYield = localText.getText("TXT_KEY_PEDIA_TECH_YIELD", (gc.getYieldInfo(k).getTextKey(), iYieldChange, gc.getYieldInfo(k).getChar(), gc.getTraitInfo(iTrait).getDescription()))
					screen.appendListBoxString( listName, szYield, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
		...
		for iBuild in range(gc.getNumBuildInfos()):
			if (gc.getBuildInfo(iBuild).getImprovement() == self.iImprovement):
				...
				if (iTech > -1):
					...
					break
		iTech2 = gc.getImprovementInfo(self.iImprovement).getPrereqTech()
		#if (iTech2 > -1):
			#BugUtil.debug("Prereq Tech For %s Improvement is %s", gc.getImprovementInfo(self.iImprovement).getDescription(), gc.getTechInfo(iTech2).getDescription())
		#else:
			#BugUtil.debug("No Prereq Tech For %s Improvement", gc.getImprovementInfo(self.iImprovement).getDescription())
		if (iTech2 > -1):
			#BugUtil.debug("Prereq Tech For %s Improvement is %s", gc.getImprovementInfo(self.iImprovement).getDescription(), gc.getTechInfo(iTech2).getDescription())
			if (iTech2 != iTech):
				#BugUtil.debug("Prereq Tech For %s Improvement is %s", gc.getImprovementInfo(self.iImprovement).getDescription(), gc.getTechInfo(iTech2).getDescription())
				screen.attachImageButton( panelName, "", gc.getTechInfo(iTech2).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TECH, iTech2, 1, False )
...
# Rise of Mankind 2.8 - show Air Defense modifier in sevopedia
		if ( gc.getImprovementInfo(self.iImprovement).getAirBombDefense() > 0):
			iAirDefense = gc.getImprovementInfo(self.iImprovement).getAirBombDefense()
			szSpecialText += localText.getText("TXT_KEY_IMPROVEMENT_ICON_BULLET", ())
			szSpecialText += str(iAirDefense)
			szSpecialText += localText.getText("TXT_KEY_PEDIA_IMPROVEMENT_AIR_DEFENSE", ())
# Rise of Mankind 2.8
# Rise of Mankind 2.9
		if ( gc.getImprovementInfo(self.iImprovement).isOutsideBorders()):
			szSpecialText += localText.getText("TXT_KEY_IMPROVEMENT_ICON_BULLET", ())
			szSpecialText += localText.getText("TXT_KEY_PEDIA_IMPROVEMENT_BUILD_OUTSIDE_BORDERS", ())
# Rise of Mankind 2.9
		screen.addMultilineText(listName, szSpecialText, self.X_EFFECTS+5, self.Y_EFFECTS+25, self.W_EFFECTS-10, self.H_EFFECTS-20, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def placeHistory(self):
		...
		panelName = self.top.getNextWidgetName()
		screen.addPanel( panelName, localText.getText("TXT_KEY_PEDIA_HISTORY", ()), "", True, True,
		...
		if ("TXT_KEY" in szText):
			szText = localText.getText("TXT_KEY_PEDIA_NO_HISTORY", ())
