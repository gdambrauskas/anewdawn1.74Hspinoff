...
# Rise of Mankind end
		...
		for j in range(gc.getNumBuildingClassInfos()):
			if (gc.getBuildingInfo(self.iBuilding).getPrereqNumOfBuildingClass(j) > 0 and not gc.getBuildingInfo(self.iBuilding).isBuildingClassNeededInCity(j)):
				iBuilding = gc.getBuildingClassInfo(j).getDefaultBuildingIndex()
				if (iBuilding > 0):
					screen.attachImageButton(panelName, "", gc.getBuildingInfo(iBuilding).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BUILDING, iBuilding, -1, False)
		for j in range(gc.getNumBuildingClassInfos()):
			if (gc.getBuildingInfo(self.iBuilding).isPrereqOrBuildingClass(j)):
				iBuilding = gc.getBuildingClassInfo(j).getDefaultBuildingIndex()
				if (iBuilding > 0):
					if (not bFirst):
						screen.attachLabel(panelName, "", localText.getText("TXT_KEY_OR", ()))
					screen.attachImageButton(panelName, "", gc.getBuildingInfo(iBuilding).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BUILDING, iBuilding, -1, False)
				bFirst = False
		#Afforess Civic Prereqs
		for j in range(gc.getNumCivicInfos()):
			if (gc.getBuildingInfo(self.iBuilding).isPrereqAndCivics(j)):
				eCivic = CivicTypes(j)
		...
		bFirst = True
		for j in range(gc.getNumCivicInfos()):
			if (gc.getBuildingInfo(self.iBuilding).isPrereqOrCivics(j)):
				if (not bFirst):
					screen.attachLabel(panelName, "", localText.getText("TXT_KEY_OR", ()))
				eCivic = CivicTypes(j)
				screen.attachImageButton(panelName, "", gc.getCivicInfo(eCivic).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIVIC, eCivic, -1, False)
		...
		for j in range(gc.getNumTerrainInfos()):
			if (gc.getBuildingInfo(self.iBuilding).isPrereqAndTerrain(j)):
				screen.attachImageButton(panelName, "", gc.getTerrainInfo(j).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TERRAIN, j, -1, False)
		bFirst = true
		for j in range(gc.getNumTerrainInfos()):
			if (gc.getBuildingInfo(self.iBuilding).isPrereqOrTerrain(j)):
				if (not bFirst):
					screen.attachLabel(panelName, "", localText.getText("TXT_KEY_OR", ()))
				screen.attachImageButton(panelName, "", gc.getTerrainInfo(j).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TERRAIN, j, -1, False)
				bFirst = false
		...
		for j in range(gc.getNumImprovementInfos()):
			if (gc.getBuildingInfo(self.iBuilding).isPrereqOrImprovement(j)):
				if (not bFirst):
					screen.attachLabel(panelName, "", localText.getText("TXT_KEY_OR", ()))
				screen.attachImageButton(panelName, "", gc.getImprovementInfo(j).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_IMPROVEMENT, j, -1, False)
				bFirst = false
				
		#Feature Prereqs
		bFirst = true
		for j in range(gc.getNumFeatureInfos()):
			if (gc.getBuildingInfo(self.iBuilding).isPrereqOrFeature(j)):
				if (not bFirst):
					screen.attachLabel(panelName, "", localText.getText("TXT_KEY_OR", ()))
				screen.attachImageButton(panelName, "", gc.getImprovementInfo(j).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_FEATURE, j, -1, False)
				bFirst = false
		#Afforess end
		...
		for iBuilding in range(gc.getNumBuildingInfos()):
			if (self.getBuildingType(iBuilding) == iBuildingType):
				if (not gc.getBuildingInfo(iBuilding).isGraphicalOnly()):
					if (not CyGame().isFinalInitialized() or CyGame().canEverConstruct(iBuilding)):
