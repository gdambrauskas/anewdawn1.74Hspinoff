...
class SevoPediaTech(CvPediaScreen.CvPediaScreen):
	...
	def __init__(self, main):
		...
		self.W_TECH_PANE = 294#300 Roamty
		...
		self.X_QUOTE_TEXT_PANE = self.X_TECH_PANE + self.W_TECH_PANE + 10
		self.Y_QUOTE_TEXT_PANE = 56
		self.W_QUOTE_TEXT_PANE = 294
		self.H_QUOTE_TEXT_PANE = 124

		self.X_QUOTE_PANE = self.X_TECH_PANE
		self.Y_QUOTE_PANE = self.Y_TECH_PANE + self.H_TECH_PANE + 10
		self.W_QUOTE_PANE = 440#self.top.R_PEDIA_PAGE - self.X_QUOTE_PANE Roamty
		self.H_QUOTE_PANE = 110

		self.X_CIVS = self.X_QUOTE_PANE + self.W_QUOTE_PANE + 10
		self.Y_CIVS = self.Y_QUOTE_PANE
		...
		self.H_SPECIAL_PANE = self.H_PREREQ_PANE#self.top.B_PEDIA_PAGE - self.Y_SPECIAL_PANE Roamty

		self.X_STRATEGY_PANE = self.X_SPECIAL_PANE
		self.Y_STRATEGY_PANE = 578
		self.W_STRATEGY_PANE = 294 
		self.H_STRATEGY_PANE = self.H_PREREQ_PANE
	...
	def placeQuoteText(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel(panelName, localText.getText("TXT_KEY_PEDIA_QUOTE", ()), "", True, False, self.X_QUOTE_TEXT_PANE, self.Y_QUOTE_TEXT_PANE, self.W_QUOTE_TEXT_PANE, self.H_QUOTE_TEXT_PANE, PanelStyles.PANEL_STYLE_BLUE50)
		screen.attachLabel(panelName, "", "  ")
		szQuoteText = gc.getTechInfo(self.iTech).getQuote()
		szQuoteTextTextWidget = self.top.getNextWidgetName()
		screen.addMultilineText(szQuoteTextTextWidget, szQuoteText, self.X_QUOTE_TEXT_PANE+5, self.Y_QUOTE_TEXT_PANE+30, self.W_QUOTE_TEXT_PANE - (5 * 2), self.H_QUOTE_TEXT_PANE - (15 * 2), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
...
#		szQuote += u"\n\n" + gc.getTechInfo(self.iTech).getCivilopedia()
		...
		screen.addMultilineText(szQuoteTextWidget, szQuote, self.X_QUOTE_PANE+5, self.Y_QUOTE_PANE+10,
		    self.W_QUOTE_PANE - (5 * 2), self.H_QUOTE_PANE - (10 * 2), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def placeStrategy(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel(panelName, localText.getText("TXT_KEY_PEDIA_STRATEGY", ()), "", True, False, self.X_STRATEGY_PANE, self.Y_STRATEGY_PANE, self.W_STRATEGY_PANE, self.H_STRATEGY_PANE, PanelStyles.PANEL_STYLE_BLUE50)
		screen.attachLabel(panelName, "", "  ")
                szStrategy = gc.getTechInfo(self.iTech).getStrategy()
		szStrategyTextWidget = self.top.getNextWidgetName()
		screen.addMultilineText(szStrategyTextWidget, szStrategy, self.X_STRATEGY_PANE+5, self.Y_STRATEGY_PANE+30,
		    self.W_STRATEGY_PANE-10, self.H_STRATEGY_PANE-35, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
