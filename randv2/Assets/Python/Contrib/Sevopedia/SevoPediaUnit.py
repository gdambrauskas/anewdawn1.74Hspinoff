...
class SevoPediaUnit:
	...
	def placeRequires(self):
		...
		#AIAndy: Building prereqs OR
		nOr = gc.getUnitInfo(self.iUnit).getPrereqOrBuildingsNum()
		szLeftDelimeter = ""
		szRightDelimeter = ""
		if (nOr > 1):
			szLeftDelimeter = localText.getText("TXT_KEY_AND", ()) + "("
		...
		bFirst = True
		for j in range(nOr):
			eBuilding = gc.getUnitInfo(self.iUnit).getPrereqOrBuilding(j)
			if (eBuilding > -1):
				if (not bFirst):
					screen.attachLabel(panelName, "", localText.getText("TXT_KEY_OR", ()))
				else:
...
# Rise of Mankind 2.9 - show some extra modifiers
		if (gc.getUnitInfo(self.iUnit).isIgnoreBuildingDefense() == True ):
			szSpecialText += localText.getText("TXT_KEY_PEDIA_UNIT_IGNORES_BUILDING_DEFENSE", ())
		if (gc.getUnitInfo(self.iUnit).getConscriptionValue() > 0):
			szSpecialText += localText.getText("TXT_KEY_PEDIA_UNIT_DRAFTABLE", ())
		if (gc.getUnitInfo(self.iUnit).getUnitCaptureClassType() > 0):
			szSpecialText += localText.getText("TXT_KEY_PEDIA_UNIT_CAN_BE_CAPTURED", ())
