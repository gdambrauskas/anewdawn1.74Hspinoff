...
class SevoPediaMain(CvPediaScreen.CvPediaScreen):
	...
	def __init__(self):
		...
		self.mapListGenerators = {
			...
			#Afforess
			SevoScreenEnums.PEDIA_BUILDING_TREE	: self.placeBuildingTree,
			#Afforess
			...
			SevoScreenEnums.PEDIA_CONCEPT_DCM  	: self.placeDCMConcepts,
			#Afforess
			SevoScreenEnums.PEDIA_CONCEPT_AND  	: self.placeANDConcepts,
			#Afforess
			}
		...
		self.mapScreenFunctions = {
			...
			SevoScreenEnums.PEDIA_CONCEPT_DCM  	: SevoPediaHistory.SevoPediaHistory(self),
			#Afforess
			SevoScreenEnums.PEDIA_CONCEPT_AND  	: SevoPediaHistory.SevoPediaHistory(self),
			#Afforess
			}
	...
	def pediaShow(self):
		...
		if (not self.pediaHistory):
			self.pediaHistory.append((SevoScreenEnums.PEDIA_MAIN, SevoScreenEnums.PEDIA_CONCEPT_AND))
	...
	def pediaJump(self, iCategory, iItem, bRemoveFwdList, bIsLink):
		...
		if (not self.pediaHistory):
			bAddToHistory = True
		elif (iCategory != SevoScreenEnums.PEDIA_MAIN or iItem == SevoScreenEnums.PEDIA_UNIT_UPGRADES or iItem == SevoScreenEnums.PEDIA_PROMOTION_TREE or iItem == SevoScreenEnums.PEDIA_HINTS or iItem == SevoScreenEnums.PEDIA_BUILDING_TREE):
		...
		if (iCategory not in (SevoScreenEnums.PEDIA_UNIT_UPGRADES, SevoScreenEnums.PEDIA_PROMOTION_TREE, SevoScreenEnums.PEDIA_HINTS, SevoScreenEnums.PEDIA_BUILDING_TREE)):
	...
	def setPediaCommonWidgets(self):
		...
		#Afforess
		self.szCategoryBuildingTree = localText.getText("TXT_KEY_PEDIA_CATEGORY_BUILDING_TREE", ())
		#Afforess
		...
		self.szCategoryDCM   		= localText.getText("TXT_KEY_PEDIA_CATEGORY_DCM", ())
		#Afforess
		self.szCategoryAND   		= localText.getText("TXT_KEY_PEDIA_CATEGORY_AND", ())
		#Afforess
		...
		self.categoryList = [
			...
			["STRATEGY",	self.szCategoryStrategy],
			["ROM",	self.szCategoryDCM],
			#Afforess
			["AND",	self.szCategoryAND],
			#Afforess
			]
		...
		self.categoryGraphics = {
			...
			"PROMOTIONS"	: u"%c  " %(CyGame().getSymbolID(FontSymbols.STAR_CHAR)),
			"PROMOTIONSTREE"	: u"%c  " %(CyGame().getSymbolID(FontSymbols.SILVER_STAR_CHAR)),
			...
			"CORPORATIONS"	: u"%c  " %(CyGame().getSymbolID(FontSymbols.TRADE_CHAR)),
			"HINTS"		: u"%c  " %(gc.getYieldInfo(YieldTypes.YIELD_COMMERCE).getChar()),
			"STRATEGY"		: u"%c  " %(CyGame().getSymbolID(FontSymbols.DEFENSIVE_PACT_CHAR)),
			"ROM"		: u"%c  " %(CyGame().getSymbolID(FontSymbols.GOLDEN_AGE_CHAR)),
			"AND"		: u"%c  " %(CyGame().getSymbolID(FontSymbols.OCCUPATION_CHAR)),
			}
	...
	def placeBuildingTree(self):
		...
		screen.addScrollPanel(self.UPGRADES_GRAPH_ID, u"", self.X_ITEMS, self.Y_PEDIA_PAGE - 13, self.W_SCREEN - self.X_ITEMS, self.H_PEDIA_PAGE + 2, PanelStyles.PANEL_STYLE_STANDARD)
		screen.setActivation(self.UPGRADES_GRAPH_ID, ActivationTypes.ACTIVATE_NORMAL)
		upgradesGraph = UnitUpgradesGraph.BuildingsGraph(self)
		upgradesGraph.getGraph()
		upgradesGraph.drawGraph()
	...
	# dcm START
	def placeDCMConcepts(self):
		self.list = self.getDCMConceptList()
		self.placeItems(WidgetTypes.WIDGET_PEDIA_DESCRIPTION, self.getDCMConceptInfo)
	
	def getDCMConceptInfo(self, id):
		info = gc.getDCMConceptInfo(id)
		if not self.isShortcutInfo(info) and not self.isStrategyInfo(info) and not self.isTraitInfo(info):
			return info
		return None

	def getDCMConceptList(self):
		return self.getSortedList(gc.getNumDCMConceptInfos(), self.getDCMConceptInfo)
	# DCM END
	
	# Afforess START
	def placeANDConcepts(self):
		self.list = self.getANDConceptList()
		self.placeItems(WidgetTypes.WIDGET_PEDIA_DESCRIPTION, self.getANDConceptInfo)
	
	def getANDConceptInfo(self, id):
		info = gc.getANDConceptInfo(id)
		if not self.isShortcutInfo(info) and not self.isStrategyInfo(info) and not self.isTraitInfo(info):
			return info
		return None

	def getANDConceptList(self):
		return self.getSortedList(gc.getNumANDConceptInfos(), self.getANDConceptInfo)
	# Afforess END
	...
	def placeItems(self, widget, info):
		...
		for item in self.list:
			if (info == gc.getConceptInfo):
				...
				data2 = item[1]
			# DCM Start
			elif (info == self.getDCMConceptInfo):
				data1 = CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_DCM
				data2 = item[1]
			# DCM end
			# Afforess Start
			elif (info == self.getANDConceptInfo):
				data1 = CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_AND
				data2 = item[1]
			# Afforess end
			else:
				data1 = item[1]
	...
	def link(self, szLink):
		...
		elif (szLink == "PEDIA_MAIN_DCM"):
			return self.pediaJump(SevoScreenEnums.PEDIA_MAIN, SevoScreenEnums.PEDIA_CONCEPT_DCM, True, True)
		#Afforess
		elif (szLink == "PEDIA_MAIN_AND"):
			return self.pediaJump(SevoScreenEnums.PEDIA_MAIN, SevoScreenEnums.PEDIA_CONCEPT_AND, True, True)
		#Afforess
...
# DCM START
		for i in range(gc.getNumDCMConceptInfos()):
			if (gc.getDCMConceptInfo(i).isMatchForLink(szLink, False)):
				return self.pediaJump(SevoScreenEnums.PEDIA_CONCEPT_DCM, i, True, True)
# DCM END
# Afforess START
		for i in range(gc.getNumANDConceptInfos()):
			if (gc.getANDConceptInfo(i).isMatchForLink(szLink, False)):
				return self.pediaJump(SevoScreenEnums.PEDIA_CONCEPT_AND, i, True, True)
# Afforess END
