...
#				return 1
			...
			'Check if ALT + T was hit == testing!'
			# Commented out start - because getXmlVal is not valid in BtS - Dancing Hoskuld
			#~ if (theKey == int(InputTypes.KB_T)
			#~ and self.eventMgr.bAlt):
...
#				Civ1 declares war on Civ2
	...
	def storeStuff(self):
		...
		for iCiv1 in range(0, ziMaxCiv, 1):
			if PyPlayer(iCiv1).isAlive():
				for iCiv2 in range(0, ziMaxCiv, 1):
					if PyPlayer(iCiv2).isAlive():
	...
	def checkStuff(self):
		...
		if (AutologOpt.isLogAttitude()):
			for iCiv1 in range(0, ziMaxCiv, 1):
				if not PyPlayer(iCiv1).isAlive():
					continue
				for iCiv2 in range(0, ziMaxCiv, 1):
					if not PyPlayer(iCiv2).isAlive():
						continue
					# Don't try to get a civ's attitude with itself
					if iCiv1 == iCiv2:
						continue
