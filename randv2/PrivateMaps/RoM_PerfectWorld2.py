...
#-----------------------------------------------------------------------------
#	2.06_mst		Temudjin 15.July.2010 - use MapScriptTools
...
def getDescription():
	...
	return "Random map that simulates earth-like plate tectonics, " +\
			 "geostrophic and monsoon winds and rainfall.  Ver. " + getVersion()
...
def beforeGeneration():
	...
	# Initialize BonusBalancer
	balancer.initialize( False, mst.iif( mst.bFFH, True, False ), True )	# don't balance, add missed boni for FFH only, move minerals
...
def addRivers():
	...
	# Build map-regions
	mst.mapRegions.buildBigBogs()									# build some bogs
	# Build map-regions
	mst.mapRegions.buildBigDents()								# build some dents

	# no rivers on Mars
	if not mst.bMars:
		# Generate rivers
		CyPythonMgr().allowDefaultImpl()								# generate rivers
...
def addFeatures():
	...
	mst.riverMaker.buildRiversFromLake( None, 40, 2, 3 )	# 40% chance each for up to 2 rivers per lake >= 3 tiles
	if mst.bPfall or mst.bMars:
		# Planetfall/Mars Now! use default featureGenerator
		featuregen = mst.MST_FeatureGenerator()				# for Planetfall uses MST_FeatureGenerator()
		featuregen.addFeatures()
	else:
		# Script has own way to generate features
		addFeatures2()													# call renamed script function
	# Build map-regions
	mst.mapRegions.buildElementalQuarter( 33 )				# build ElementalQuarter 33% chance
...
def normalizeStartingPlotLocations():
	...
	if CyMap().getCustomMapOption(3) == 0:						# by default civ places teams near to each other
		CyPythonMgr().allowDefaultImpl()
		# mst.teamStart.placeTeamsTogether( True, False )
	elif CyMap().getCustomMapOption(3) == 1:					# shuffle starting-plots to separate teams
		mst.teamStart.placeTeamsTogether( False, True )
	else:
		mst.teamStart.placeTeamsTogether( True, True )		# randomize starting-plots to ignore teams

def normalizeAddExtras():
	print "--- normalizeAddExtras()"
	# Place missing boni, move minerals
...
#	normalizeAddExtras2()											# call renamed script function
	...
	# Place special features on map if mod supports them
...
########## Temudjin START
	bigGrids = [ (42,26), (46,30) ]							# just change these, or even add some more
...
##### Temudjin START
    if mst.bPfall:
        ...
        # translate already generated terrain
        terrainTypes = [0]*CyMap().numPlots()
        terrainList = [0, 1, 5, 4, 3, 2, 6, 7]		# Ocean, Coast, Desert, Plains, Grass, Marsh, Tundra, Snow
        for i in range( CyMap().numPlots() ):
            terrainTypes[i] = mst.planetFallMap.mapPfallTerrain( sm.terrainMap[i], terrainList, CyMap().plotByIndex(i) )
...
##### Temudjin START - use default river-generation instead
'''
...
def assignStartingPlots():
	...
	if mst.bPfall:											# Planetfall uses default starting-plot function,
		CyPythonMgr().allowDefaultImpl()				# otherwise would ignore 'scattered landing pods' option
		return
	########### Temudjin END
	gc = CyGlobalContext()
	gameMap = CyMap()
