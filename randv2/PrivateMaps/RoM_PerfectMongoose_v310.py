...
class ClimateMap3:
		...
		for zone in range(6):
			...
			if not (topY == -1 and bottomY == -1):
				...
				while y <= yStop:
					...
					while xx <= xStop - incX:
						i = em.GetIndex(xx, y)
						if em.IsBelowSeaLevel(xx, y):
							xxStart = xx
							xxStop = xx + em.width * incX
							break
						xx += incX
					x = xxStart
					while x <= xxStop - incX:
	...
	def DistributeRain(self, x, y, pressureMap, RainfallMap, moistureMap, boolGeostrophic):
		...
		if len(nList) == 0 or (boolGeostrophic and len(nList) == 1):
			...
			return
		moisturePerNeighbor = moistureMap.data[i] / len(nList)
...
class TerrainMap:
	...
	def GenerateTerrainMap(self):
		...
		self.desertThreshold = FindValueFromPercent(cm.RainfallMap.data, mc.width, mc.height, desertPercent, desertPercent / 10.0, False)
		self.plainsThreshold = FindValueFromPercent(cm.RainfallMap.data, mc.width, mc.height, plainsPercent, plainsPercent / 10.0, False)
		for y in range(mc.height):
			for x in range(mc.width):
				i = GetIndex(x, y)
				if self.data1[i] == mc.OCEAN:
				...
				#based on how close to the threshold the rainfall is
					...
					if cm.TemperatureMap.data[i] < snowTemp:
						...
						if cm.RainfallMap.data[i] < ((PRand.random() * (self.plainsThreshold - self.desertThreshold) + self.plainsThreshold - self.desertThreshold)) / 2.0 + self.desertThreshold: 
							self.data2[i] = mc.PLAINS
						else:
...
def GenerateMountainMap(width, height, wrapX, wrapY, initFreq):
	...
	for y in range(mountainMap.height):
		for x in range(mountainMap.width):
			...
			odd = y % 2.0
			xx = x + odd * 0.5
			mountainMap.data[i] = GetPerlinNoise(xx, y, mountainMap.width, mountainMap.height, initFreq, 1.0, 0.4, 8, inputNoise)
...
class StartingPlotFinder:
	...
	def getPlotPotentialValue(self, x, y, coastalCity):
		...
		for n in range(gc.getNumBuildInfos()):
			...
			if buildInfo.getTechPrereq() == TechTypes.NO_TECH or gc.getTechInfo(buildInfo.getTechPrereq()).getEra() > max(game.getStartEra(), 1):
				continue
			if plot.canHaveImprovement(impEnum, TeamTypes.NO_TEAM, True):
				...
				if featureEnum != FeatureTypes.NO_FEATURE and buildInfo.isFeatureRemove(featureEnum):
					if gc.getTechInfo(buildInfo.getFeatureTech(featureEnum)).getEra() <= max(game.getStartEra(), 1):
...
def getDescription():
	return "Official Civ4 Port of Civ5's PerfectWorld 3, with new Perlin Noise landmass generator and optional new climate system. River, Lake, Feature and Resource placement remain unchanged from PerfectWorld 2."
...
def addFeatures():
	...
	for y in range(mc.height):
		for x in range(mc.width):
			...
			plot = mmap.plot(x, y)
			#forest and jungle
			if not plot.isWater() and tm.data2[i] != mc.DESERT and not plot.isPeak():
			...
			elif tm.data2[i] == mc.DESERT and tm.data1[i] != mc.HILLS and tm.data1[i] != mc.PEAK and not plot.isWater():
				if plot.isRiver():
					...
					for yy in range(y - 1, y + 2):
						for xx in range(x - 1, x + 2):
							...
							if (tm.data2[ii] != mc.DESERT and tm.data1[ii] != mc.PEAK):
								foundNonDesert = True
							elif surPlot == 0:
								foundNonDesert = True
