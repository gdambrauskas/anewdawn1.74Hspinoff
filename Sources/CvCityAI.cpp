...
/*                                                                                              */
/************************************************************************************************/
	 if (!bLandWar && !bDanger && !bFinancialTrouble && !bDevelopingCity)
	{
		if (!GET_TEAM(getTeam()).isMapTrading())
		{
	...
	if (bDevelopingCity && !bDanger)
	{
		int iDevelopingCityFlags = 0;
		iDevelopingCityFlags |= BUILDINGFOCUS_FOOD;
		
		//Try to find a building we can build really quickly.
		int iTurns = 5 * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getConstructPercent();
		iTurns /= 100;
		if (bDefenseWar)
		{
			iTurns /= 2;
		...
		if (bFinancialTrouble)
		{
			if (AI_chooseBuilding(BUILDINGFOCUS_GOLD, iTurns))
			{
				if( gCityLogLevel >= 2 ) logBBAI("      City %S uses is in financial trouble! It pushes production of gold building", getName().GetCString());
				return;
			}
		}
		for (int iPass = 0; iPass < 4; iPass++)
		{
			if (AI_chooseBuilding(iDevelopingCityFlags, iTurns))
			{
				return;
			}
			if (AI_chooseBuilding(iDevelopingCityFlags, iTurns * 2))
			{
				return;
			}
			if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_REVOLUTION))
			{
				if (AI_chooseBuilding(iDevelopingCityFlags, iTurns * 3))
				{
					return;
				}
			}
			if (iPass == 0)
				iDevelopingCityFlags |= BUILDINGFOCUS_PRODUCTION;
			if (iPass == 1)
				iDevelopingCityFlags |= BUILDINGFOCUS_GOLD;
			if (iPass == 2)
				iDevelopingCityFlags |= BUILDINGFOCUS_CULTURE;
			if (iPass == 3)
				iDevelopingCityFlags |= BUILDINGFOCUS_RESEARCH;
		}
	}
	else if (!bDevelopingCity && !bLandWar && !bDefenseWar)
	{
		if (AI_buildCaravan())
		{
			if( gCityLogLevel >= 2 ) logBBAI("      City %S builds a trade caravan", getName().GetCString());
			return;
		}
	}
	//are we falling behind in research?
	bool bNeedMoreScience = false;
	int iResearchRank = 0;
	int iOthers = 0;
	int iResearchPercent = GET_PLAYER(getOwnerINLINE()).getCommercePercent(COMMERCE_RESEARCH);
	int iResearch = GET_PLAYER(getOwnerINLINE()).getCommerceRate(COMMERCE_RESEARCH);
	iResearch *= 100;
	iResearch /= std::max(5, iResearchPercent);
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		{	
			iOthers++;
			int iPlayerResearch = GET_PLAYER((PlayerTypes)iI).getCommerceRate(COMMERCE_RESEARCH);
			iPlayerResearch *= 100;
			iPlayerResearch /= std::max(5, GET_PLAYER((PlayerTypes)iI).getCommercePercent(COMMERCE_RESEARCH));
			if (iPlayerResearch > iResearch)
			{
				iResearchRank++;
				if (GET_TEAM(getTeam()).AI_getWarPlan(GET_PLAYER((PlayerTypes)iI).getTeam()) != NO_WARPLAN)
				{
					bNeedMoreScience = true;
				}
			}
		}
	}
	int iRank = GC.getGameINLINE().getPlayerRank(getOwnerINLINE());
	//Lower rank is better
	if (iRank < iResearchRank)
	{
		bNeedMoreScience = true;
	}
	if (iResearchRank > 0)
	{
		if ((iOthers / 2) < iResearchRank )
		{
			bNeedMoreScience = true;
		}
	}
	//XXX When else do we need more research?
	if (bNeedMoreScience)
	{
		int iTurns = 7 * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getConstructPercent();
		iTurns /= 100;
		if (AI_chooseBuilding(BUILDINGFOCUS_RESEARCH, iTurns))
		{
			return;
		}
		if (AI_chooseBuilding(BUILDINGFOCUS_RESEARCH, iTurns * 2))
		{
			return;
		}
	}
...
/*                                                                                              */
/* This causes the AI to build so many weak units it's economy is destroyed.                    */
/* The AI should only build lots of units if it have the money, or is in danger                 */
/************************************************************************************************/
/*
	if( bLandWar && (iWarSuccessRatio < -30 || iEnemyPowerPerc > 150) )
*/
	if (( bLandWar && (iWarSuccessRatio < -30 || iEnemyPowerPerc > 150)) && (bDanger || !bFinancialTrouble))
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
	{
...
*/
		if (( bDefenseWar || (bLandWar && (iWarSuccessRatio < -30)) ) && (bDanger || !bFinancialTrouble))
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
		{
...
/************************************************************************************************/
	...
	for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		aiFreeSpecialistYield[iI] = 0;
	}

	int aiFreeSpecialistCommerce[NUM_COMMERCE_TYPES];
	for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		aiFreeSpecialistCommerce[iI] = 0;
	}
...
/************************************************************************************************/
				if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_REVOLUTION))
				{
					...
					{
						iValue += kBuilding.getRevIdxLocal() * (getLocalRevIndex() / 50);
					}
					...
					if (kBuilding.getRevIdxDistanceModifier() != 0)
					{
						if (!isCapital())
						{
							...
							if (pCapital != NULL)
							{
								...
								iValue += ((iOldCapitalDistance - iCapitalDistance) * 20);
							}
						}
					}
				}
...
/************************************************************************************************/
			...
			if ((iFocusFlags & BUILDINGFOCUS_EXPERIENCE) || (iPass > 0))
			{
				...
				{
					bool bRequireBonus = false;
					bool bCreatesReqBonus = false;
					for (iI = 0; iI < GC.getNumUnitInfos(); iI++)
					{
						...
						if (kBuilding.getFreeBonus() != NO_BONUS)
						{
							for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
							{
								...
								if (!hasBonus(eBonus))
								{
									...
									{
										if (kUnit.getPrereqAndBonus() == eBonus || kUnit.getPrereqOrBonuses(iK) == eBonus)
										{
											bRequireBonus = true;
											if (kBuilding.getFreeBonus() == eBonus)
											{
												bCreatesReqBonus = true;
											}
										}
									}
								}
						...
						if (GC.getUnitInfo((UnitTypes)iI).getPrereqBuilding() == eBuilding || (bRequireBonus && bCreatesReqBonus))
						{
...
/************************************************************************************************/
				...
				if (kBuilding.getPopulationgrowthratepercentage() != 0)
				{
					...
					iTempValue += -(kBuilding.getPopulationgrowthratepercentage() * iHappinessLevel / 10);

					else

					iTempValue += -(kBuilding.getPopulationgrowthratepercentage() * iHappinessLevel / 8);
				...
				if (kBuilding.getGlobalPopulationgrowthratepercentage() != 0)
				{
					...
					if (iCityHappy > 0)
						iTempValue += -(kBuilding.getGlobalPopulationgrowthratepercentage() * iCityHappy / 10);

					else
						iTempValue += -(kBuilding.getGlobalPopulationgrowthratepercentage() * iCityHappy / 8);
...
/*                                                                                              */
/************************************************************************************************/
					iTempValue = (((kBuilding.getCommerceModifier(COMMERCE_GOLD) + GET_TEAM(getTeam()).getBuildingCommerceModifier(eBuilding, COMMERCE_GOLD)) * getBaseCommerceRate(COMMERCE_GOLD)) / 40);
					/* Was:
...
/*                                                                                              */
/************************************************************************************************/
					iTempValue = (((kBuilding.getCommerceModifier(COMMERCE_RESEARCH) + GET_TEAM(getTeam()).getBuildingCommerceModifier(eBuilding, COMMERCE_RESEARCH)) * getBaseCommerceRate(COMMERCE_RESEARCH)) / 40);
					/* Was:
...
/*                                                                                              */
/************************************************************************************************/
					iValue += (((kBuilding.getCommerceModifier(COMMERCE_CULTURE) + GET_TEAM(getTeam()).getBuildingCommerceModifier(eBuilding, COMMERCE_CULTURE)) * getBaseCommerceRate(COMMERCE_CULTURE)) / 15);
...
/*                                                                                              */
/************************************************************************************************/
					iTempValue = (((kBuilding.getCommerceModifier(COMMERCE_ESPIONAGE) + GET_TEAM(getTeam()).getBuildingCommerceModifier(eBuilding, COMMERCE_ESPIONAGE)) * getBaseCommerceRate(COMMERCE_ESPIONAGE)) / 80);
					/* Was:
					iTempValue = ((kBuilding.getCommerceModifier(COMMERCE_ESPIONAGE) * getBaseCommerceRate(COMMERCE_ESPIONAGE)) / 80);
					*/
					for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
					{
						iTempValue += ((kBuilding.getBonusCommerceModifier(iJ,COMMERCE_ESPIONAGE) * getBaseCommerceRate(COMMERCE_ESPIONAGE)) / 80);
					}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/	
...
int CvCityAI::AI_getImprovementValue( CvPlot* pPlot, ImprovementTypes eImprovement, int iFoodPriority, int iProductionPriority, int iCommercePriority, int iFoodChange, bool bOriginal )
{
	...
	if (eNonObsoleteBonus != NO_BONUS)
	{
		...
		{
/*************************************************************************************************/
/**	Xienwolf Tweak							02/12/10						Imported: Afforess  **/
...
/*************************************************************************************************/
/**								---- Start Original Code ----									**
			if (GC.getImprovementInfo((ImprovementTypes)pPlot->getImprovementType()).isImprovementBonusTrade(eNonObsoleteBonus))
/**								----  End Original Code  ----									**/
			if (GC.getImprovementInfo((ImprovementTypes)pPlot->getImprovementType()).isImprovementBonusTrade(eNonObsoleteBonus) || GC.getImprovementInfo((ImprovementTypes)pPlot->getImprovementType()).isActsAsCity())
/*************************************************************************************************/
/**	Tweak									END													**/
/*************************************************************************************************/
			{
...
*/
					if ((aiDiffYields[YIELD_PRODUCTION] > 0) && ((aiFinalYields[YIELD_FOOD]+aiFinalYields[YIELD_PRODUCTION] > 5) 
					|| ((pPlot->isHills() || pPlot->isPeak()) && aiDiffYields[YIELD_PRODUCTION] > 2)))
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
					{
...
/*                                                                                              */
/************************************************************************************************/	
		iValue += 75 * GC.getSpecialistInfo(eSpecialist).getHealth();
		iValue += 75 * GC.getSpecialistInfo(eSpecialist).getHappiness();
/************************************************************************************************/
/* Specialists Enhancements                          END                                        */
/************************************************************************************************/
...
/************************************************************************************************/
/* Afforess	                  Start		 07/23/10                                               */
/*                                                                                              */
/*                                                                                              */
/************************************************************************************************/
		if (GET_PLAYER(getOwnerINLINE()).AI_isFinancialTrouble())
		{
			iMultiplier += GET_PLAYER(getOwnerINLINE()).getCommercePercent(COMMERCE_GOLD) / 5;
		}
	}
	if (eYield == YIELD_FOOD)
	{
		int iHappy = happyLevel() - unhappyLevel(1);
		if (iHappy > 0)
		{
			iMultiplier += 6 * iHappy;
		}
	}
/************************************************************************************************/
/* Afforess	                     END                                                            */
...
/*       Divide By Zero Fix                                                                     */
/************************************************************************************************/
		int iIncome = std::max(1, 1 + kPlayer.getCommerceRate(COMMERCE_GOLD) + kPlayer.getCommerceRate(COMMERCE_RESEARCH) + std::max(0, kPlayer.getGoldPerTurn()));
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/	
