...
/************************************************************************************************/
		...
		{
			CvCity* pCity = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCity(info.getData1());
			CvMessageControl::getInstance().sendDoTask(info.getData1(), TASK_SACK_CITY, GC.getGameINLINE().getActivePlayer(), -1, false, false, false, false);
		}
		//Enslave City
		else if (pPopupReturn->getButtonClicked() == 5)
		{
			...
			CvMessageControl::getInstance().sendDoTask(info.getData1(), TASK_ENSLAVE_CITY, GC.getGameINLINE().getActivePlayer(), -1, false, false, false, false);
		}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
/*                                                                                              */
/*No Future Era and Advanced Economy Options Alter the Game Data. A full reload it needed after */
/* Either of these options have been selected. Re-Loading XML after the game has initialized    */
/* Doesn't work.                                                                                */
/************************************************************************************************/
/*
			else if (!gDLL->getInterfaceIFace()->isDebugMenuCreated())
*/
			else if (GC.getGameINLINE().canExitToMainMenu())
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
			{
...
/*                                                                                              */
/************************************************************************************************/
	case BUTTONPOPUP_ELECTION_LANDSLIDE:
		GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setElectionResults(true);
		break;
		
	case BUTTONPOPUP_ELECTION_VICTORY:
		GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setElectionResults(true);
		break;
		
	case BUTTONPOPUP_ELECTION_DEFEAT:
		GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setElectionResults(false);
		break;
		
	case BUTTONPOPUP_ELECTION_FAKE_VICTORY:
		GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setElectionResults(true);
		break;
	
	case BUTTONPOPUP_ELECTION_DEFEAT_FRAUD:
		GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setElectionResults(false);
		break;
		
	case BUTTONPOPUP_ELECTION:
		{
			int iLastElectionTimer = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getLastElectionTimer();
			int iElectionResults = GC.getGameINLINE().getSorenRandNum(100, "Election Results");
			//Regular Election Continues
			if (pPopupReturn->getButtonClicked() == 0)
			{
				if (iElectionResults >= (int)info.getData1())
				{
					CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_ELECTION_VICTORY, iElectionResults, info.getData2());
					gDLL->getInterfaceIFace()->addPopup(pInfo, GC.getGameINLINE().getActivePlayer(), false, true);
				}
				else
				{
					CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_ELECTION_DEFEAT, iElectionResults, info.getData2());
					gDLL->getInterfaceIFace()->addPopup(pInfo, GC.getGameINLINE().getActivePlayer(), false, true);
				}
				GET_PLAYER(GC.getGameINLINE().getActivePlayer()).changeLastElectionTimer(-iLastElectionTimer);
			}
			//Fraud the Election
			else if (pPopupReturn->getButtonClicked() == 1)
			{
				GET_PLAYER(GC.getGameINLINE().getActivePlayer()).changeGold(-100);
				int iFraud = GC.getGameINLINE().getSorenRandNum(100, "Election Fraud");
				if (iFraud >= GC.getDefineINT("ELECTION_FRAUD_DETECTION"))
				{
					//Fraud Detected
					iFraud -= GC.getGameINLINE().getSorenRandNum(200, "Election Defrauded");
				}
				if (iElectionResults + iFraud >= (int)info.getData1())
				{
					CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_ELECTION_FAKE_VICTORY, iElectionResults, info.getData2(), iFraud);
					gDLL->getInterfaceIFace()->addPopup(pInfo, GC.getGameINLINE().getActivePlayer(), false, true);
				}
				else
				{
					CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_ELECTION_DEFEAT_FRAUD, iElectionResults, info.getData2(), iFraud);
					gDLL->getInterfaceIFace()->addPopup(pInfo, GC.getGameINLINE().getActivePlayer(), false, true);
				}
				GET_PLAYER(GC.getGameINLINE().getActivePlayer()).changeLastElectionTimer(-iLastElectionTimer);
			}
			...
			{
				GET_PLAYER(GC.getGameINLINE().getActivePlayer()).changeElectionAnger(GC.getDefineINT("ELECTION_DELAY_ANGER"));
				GET_PLAYER(GC.getGameINLINE().getActivePlayer()).changeLastElectionTimer(-GC.getDefineINT("ELECTION_DELAY_LENGTH"));
			}
			//Cancel the Election
			else if (pPopupReturn->getButtonClicked() == 3)
			{
				GET_PLAYER(GC.getGameINLINE().getActivePlayer()).makeMeKing(true);
				GET_PLAYER(GC.getGameINLINE().getActivePlayer()).changeLastElectionTimer(-iLastElectionTimer);
			}
		}
		break;
	case BUTTONPOPUP_CHOOSE_CURRENCY:
		GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setCurrency((BonusTypes)pPopupReturn->getButtonClicked());
		gDLL->getInterfaceIFace()->playGeneralSound("AS2D_WELOVEKING");
		break;
...
/************************************************************************************************/
	case BUTTONPOPUP_ELECTION_LANDSLIDE:
		bLaunched = electionLandslideVictory(pPopup, info);
		break;
		
	case BUTTONPOPUP_ELECTION_VICTORY:
		bLaunched = electionVictory(pPopup, info);
		break;
		
	case BUTTONPOPUP_ELECTION_DEFEAT:
		bLaunched = electionDefeat(pPopup, info);
		break;
		
	case BUTTONPOPUP_ELECTION_FAKE_VICTORY:
		bLaunched = electionFakeVictory(pPopup, info);
		break;
	
	case BUTTONPOPUP_ELECTION_DEFEAT_FRAUD:
		bLaunched = electionFraudDefeat(pPopup, info);
		break;
		
	case BUTTONPOPUP_ELECTION:
		bLaunched = election(pPopup, info);
		break;
	case BUTTONPOPUP_CHOOSE_CURRENCY:
		bLaunched = chooseCurrency(pPopup, info);
		break;
	case BUTTONPOPUP_INVASION:
...
/*                                                                                              */
/*  Sort Regular Buildings and Wonders                                                          */
/************************************************************************************************/
/*
...
*/
	...
	for (int iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			if (eLoopBuilding != eProductionBuilding)
			{
				if (!isNationalWonderClass((BuildingClassTypes)iI) && isLimitedWonderClass((BuildingClassTypes)iI))
				{
					if (pCity->canConstruct(eLoopBuilding))
					{
						int iTurns = pCity->getProductionTurnsLeft(eLoopBuilding, 0);
...
/************************************************************************************************/
		...
		{
			if (player.canSack(pNewCity))
			{
				gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_SACK_CITY").c_str(), NULL, 4, WIDGET_GENERAL);
			}
			if (player.canEnslave(pNewCity))
			{
				gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_ENSLAVE_CITY").c_str(), NULL, 5, WIDGET_GENERAL);
			}
		}
/************************************************************************************************/
/* Afforess	                     END                                                            */
...
*/
	if (GC.getGameINLINE().canExitToMainMenu())
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
	{
...
/*                                                                                              */
/************************************************************************************************/
	if (!GET_PLAYER(ePlayer).m_bChoosingReligion)
	{
...
/*                                                                                              */
/************************************************************************************************/
bool CvDLLButtonPopup::electionLandslideVictory(CvPopup* pPopup, CvPopupInfo &info)
{
	CvPlayer& player = GET_PLAYER(GC.getGameINLINE().getActivePlayer());

	CvWString szBuffer;
	
	szBuffer = gDLL->getText("TXT_KEY_POPUP_LANDSLIDE_VICTORY");
	
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_BACK_TO_BUSINESS").c_str(), NULL, 0, WIDGET_GENERAL);

	gDLL->getInterfaceIFace()->popupLaunch(pPopup, false, POPUPSTATE_IMMEDIATE);
	gDLL->getInterfaceIFace()->playGeneralSound("AS2D_CIVIC_ADOPT");

	return (true);
}

bool CvDLLButtonPopup::electionVictory(CvPopup* pPopup, CvPopupInfo &info)
{
	CvPlayer& player = GET_PLAYER(GC.getGameINLINE().getActivePlayer());

	CvWString szBuffer;
	
	szBuffer = gDLL->getText("TXT_KEY_POPUP_ELECTION_VICTORY", (int)info.getData1());
...
bool CvDLLButtonPopup::electionDefeat(CvPopup* pPopup, CvPopupInfo &info)
{
	CvPlayer& player = GET_PLAYER(GC.getGameINLINE().getActivePlayer());

	CvWString szBuffer;
	
	szBuffer = gDLL->getText("TXT_KEY_POPUP_ELECTION_DEFEAT", (100 - (int)info.getData1()));
	
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_THATS_TERRIBLE").c_str(), NULL, 0, WIDGET_GENERAL);
...
bool CvDLLButtonPopup::electionFakeVictory(CvPopup* pPopup, CvPopupInfo &info)
{
	CvPlayer& player = GET_PLAYER(GC.getGameINLINE().getActivePlayer());

	CvWString szBuffer;
	
	szBuffer = gDLL->getText("TXT_KEY_POPUP_ELECTION_VICTORY_FAKE", (int)info.getData1(), (int)info.getData3());
...
bool CvDLLButtonPopup::electionFraudDefeat(CvPopup* pPopup, CvPopupInfo &info)
{
	CvPlayer& player = GET_PLAYER(GC.getGameINLINE().getActivePlayer());

	CvWString szBuffer;
	
	szBuffer = gDLL->getText("TXT_KEY_POPUP_ELECTION_FRAUD_DEFEAT");
...
/*
Sire, the Election is Approaching, what should we do?

Proceed with the Elections, Let Democracy run it's course
Fraud the Elections
Delay the Elections
Screw Elections, Make me King!
*/	
bool CvDLLButtonPopup::election(CvPopup* pPopup, CvPopupInfo &info)
{
	CvPlayer& player = GET_PLAYER(GC.getGameINLINE().getActivePlayer());

	CvWString szBuffer;
	
	szBuffer = gDLL->getText("TXT_KEY_POPUP_ELECTION");
	
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	
	
	bool bKingMaker = player.makeMeKing(false);
	
	int iFraudCost = (((player.getGold() / 10) + GC.getGameINLINE().getSorenRandNum(1000, "Election Fraud Cost")) * (200 - (int)info.getData1())) / 100;
	bool bCanFraud = false;
	if (player.getGold() > iFraudCost)
		bCanFraud = true;


	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_ELECTION_NORMAL").c_str(), NULL, 0, WIDGET_GENERAL);

	if (bCanFraud)
	{
		gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_FRAUD_ELECTION").c_str(), NULL, 1, WIDGET_GENERAL);
	}
	gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_DELAY_ELECTION").c_str(), NULL, 2, WIDGET_GENERAL);
	if (bKingMaker)
	{
		gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_FRAUD_ELECTION").c_str(), NULL, 3, WIDGET_GENERAL);
	}
...
bool CvDLLButtonPopup::chooseCurrency(CvPopup* pPopup, CvPopupInfo &info)
{
	CvPlayer& kPlayer = GET_PLAYER(GC.getGameINLINE().getActivePlayer());
	CvWString szBuffer;
	
	//Body of the Message
	szBuffer = gDLL->getText("TXT_KEY_POPUP_CHOOSE_CURRENCY");
	gDLL->getInterfaceIFace()->popupSetBodyString(pPopup, szBuffer);
	
	int iCount = 0;
	
	//Always Check the favorites first
	int iResource = GC.getInfoTypeForString("BONUS_GOLD");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
			gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, GC.getBonusInfo((BonusTypes)iResource).getDescription(), GC.getBonusInfo((BonusTypes)iResource).getButton(), iResource, WIDGET_PEDIA_JUMP_TO_BONUS);
			iCount++;
		}
	}
	iResource = GC.getInfoTypeForString("BONUS_SILVER");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_COPPER");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_IRON");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_GEMS");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_INCENSE");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_IVORY");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_SILK");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_SPICES");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_SUGAR");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_SALT");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_COFFEE");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_PEARLS");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_OBSIDIAN");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
	...
	iResource = GC.getInfoTypeForString("BONUS_LEAD");
	if (iResource > 0)
	{
		if (kPlayer.getNumTradeableBonuses((BonusTypes)iResource) > 0)
		{
