...
class SevoPediaIndex:
	...
	def buildIndex(self):
		...
		DCMConceptList = self.top.getDCMConceptList()
		
		##Afforess
		ANDConceptList = self.top.getANDConceptList()
		##Afforess
		...
		for item in DCMConceptList:
			list.append([item[0],"RoMConcept",item])
		##Afforess
		for item in ANDConceptList:
			list.append([item[0],"ANDConcept",item])
		##Afforess
	...
	def placeIndex(self):
		...
		for name, type, item in self.index:
			...
			elif (type == "RoMConcept"):
				screen.setTableText(self.tableName, iColumn, iRow, u"<font=3>%c %s</font>" % (CONCEPT_CHAR, item[0]), gc.getDCMConceptInfo(item[1]).getButton(), WidgetTypes.WIDGET_PEDIA_DESCRIPTION, CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_DCM, item[1], CvUtil.FONT_LEFT_JUSTIFY)
			##Afforess
			elif (type == "ANDConcept"):
				screen.setTableText(self.tableName, iColumn, iRow, u"<font=3>%c %s</font>" % (CONCEPT_CHAR, item[0]), gc.getANDConceptInfo(item[1]).getButton(), WidgetTypes.WIDGET_PEDIA_DESCRIPTION, CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_AND, item[1], CvUtil.FONT_LEFT_JUSTIFY)
