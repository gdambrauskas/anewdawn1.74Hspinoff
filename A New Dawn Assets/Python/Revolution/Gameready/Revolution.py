...
class Revolution :
	...
	def __init__(self, customEM, RevOpt):
		...
		self.stopAutoForRevolt = RevOpt.isStopAutoForRevolt()
		self.reinstateOnRevolt = RevOpt.isReinstateOnRevolt()
		...
		self.deniedTurns = RevOpt.getDeniedRevs()
...
# Rise of Mankind 2.9
			...
			if( not capital == None and not pCity.isConnectedTo(capital) ) :
				if( recentlyAcquired or pPlayer.isRebel() ) :
					...
					connIdx = min([2 + pPlayer.getCurrentEra() + pCity.getPopulation()/3, 6])
...
#-------------------------------------------------------------------------------------------------
	...
	def updateCivStability( self, iGameTurn, iPlayer, bIsRevWatch = False, bVerbose = False ) :
		...
		#Afforess Dark Ages
		if( pPlayer.isDarkAge() ) :
			if( self.LOG_DEBUG ) : CvUtil.pyPrint("  Revolt - The %s are in their Dark age!"%(pPlayer.getCivilizationDescription(0)))
			if( bIsRevWatch ) : negList.append( (-10, localText.getText("TXT_KEY_DARK_AGE",())) )
			civRevIdx += 20
			civStabilityIdx -= 10
			revIdxHistEvents += 30
		
		#Afforess Dark Ages End
...
#-------------------------------------------------------------------------------------------------		
		...
		if( self.isHumanPlayerOrAutoPlay(pPlayer.getID()) and game.getAIAutoPlay(pPlayer.getID()) > 0 ) :
...
#-------------------------------------------------------------------------------------------------		
			...
			#game.setAIAutoPlay( iPlayerID, 2 ) # Releases this player turn to the AI, human player changed below so that human now will control rebels
#-------------------------------------------------------------------------------------------------
# END Lemmy101 RevolutionMP edit
#-------------------------------------------------------------------------------------------------		
		else :
...
#*************************************
					...
					if( self.isLocalHumanPlayer(joinPlayer.getID()) ) :
						...
						buttons += ('goaway',)
						popup.setUserData( (buttons, iPlayer, iRevoltIdx) )
						# Center camera on city
						CyCamera().JustLookAt( cityList[0].plot().getPoint() )
						popup.launch(bCreateOkButton = False)
...
#-------------------------------------------------------------------------------------------------		
		...
		iSettler = gc.getCivilizationInfo( pRevPlayer.getCivilizationType() ).getCivilizationUnits(settlerClass)
		scoutClass = CvUtil.findInfoTypeNum(gc.getUnitClassInfo,gc.getNumUnitClassInfos(),RevDefs.sXMLScout)
		iScout = gc.getCivilizationInfo( pRevPlayer.getCivilizationType() ).getCivilizationUnits(scoutClass)
